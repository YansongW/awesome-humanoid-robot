...
return out;}
async function loadIndex(){if(!resultsList)return;resultsList.innerHTML=`<div class="loading-state">${escapeHtml(loadingMessage)}</div>`;try{const res=await fetch(basePath+'/data/search-index.json');if(!res.ok)throw new Error('Failed to load search index');searchData=await res.json();if(!searchData.entries)searchData.entries=[];if(!searchData.index)searchData.index={};init();}catch(err){resultsList.innerHTML=`<div class="empty-state">${escapeHtml(err.message)}</div>`;}}
...
return counts;}
function scoreEntry(e,q,qTokens,candidateMatchCount){let score=0;let strongMatches=0;const name=(e.name||'').toLowerCase();const nameEn=(e.name_en||'').toLowerCase();const summary=(e.summary||'').toLowerCase();const eid=(e.id||'').toLowerCase();const type=(e.type||'').toLowerCase();if(q){if(name===q){score+=600;strongMatches+=2;}
...
for(const t of qTokens){if(t.length===0)continue;const isLatin=/^[a-z0-9]+$/.test(t);const WORD_RE=/[^a-z0-9\u4e00-\u9fff\uac00-\ud7af]+/;const inName=name.split(WORD_RE).includes(t)||name.includes(t);const inNameEn=nameEn.split(WORD_RE).includes(t)||nameEn.includes(t);const inId=eid.split(/[^a-z0-9_]+/).includes(t)||eid.includes(t);const inSummary=summary.includes(t);const inType=type.includes(t);const inDomain=e.domains&&e.domains.some(d=>d.toLowerCase().includes(t));if(isLatin){const whole=inName||inNameEn||inId||inType||inDomain||summary.split(WORD_RE).includes(t);if(whole){if(inName){score+=20;strongMatches++;}
...
score+=candidateMatchCount*15;if(summary.length>20)score+=2;const primaryName=name||nameEn||eid;if(primaryName.length>80)score-=10;if(primaryName.length>140)score-=15;if(strongMatches===0&&!(q&&summary.includes(q)))return 0;return score;}
...
const qTokens=uniqueTokens(q);const candidates=findCandidates(qTokens);const scores=[];for(const[idx,matchCount]of candidates.entries()){const e=searchData.entries[idx];if(!e)continue;if(typeFilter!=='all'&&e.type!==typeFilter)continue;if(activeDomain&&!(e.domains||[]).includes(activeDomain))continue;const score=scoreEntry(e,q,qTokens,matchCount);if(score>0)scores.push({entry:e,score});}
if(scores.length===0&&q){for(const e of searchData.entries){if(typeFilter!=='all'&&e.type!==typeFilter)continue;if(activeDomain&&!(e.domains||[]).includes(activeDomain))continue;const score=scoreEntry(e,q,qTokens,0);if(score>0)scores.push({entry:e,score});}}
...
function performSearch(append=false){const query=searchInput?searchInput.value:'';const results=search(query,activeType);renderResults(results,query,append);}
...
performSearch();}else{if(resultsTitle)resultsTitle.textContent='';if(resultsList)resultsList.innerHTML=`<div class="empty-state">${escapeHtml(emptyMessage)}</div>`;if(resultsCount)resultsCount.textContent=formatCount(0);updateLoadMore();}
...
if(searchForm){searchForm.addEventListener('submit',(e)=>{e.preventDefault();const q=searchInput?searchInput.value:'';setUrlQuery(q);if(resultsTitle)resultsTitle.textContent=q?`“${q}”`:'';performSearch();if(searchInput)searchInput.blur();});}
...
if(loadMoreBtn){loadMoreBtn.addEventListener('click',()=>{renderResults(currentResults,currentQuery,true);});}}
loadIndex();})();