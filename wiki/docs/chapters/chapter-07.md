...
电机是把电能转化为机械能的能量转换装置。对于人形机器人关节常用的永磁同步电机（PMSM）与无刷直流电机（BLDC），其电磁转矩可由**转矩常数（torque constant）** $K_t$ 与交轴电流 $I_q$ 直接关联：

$$
T_e = K_t \cdot I_q
$$

其中 $T_e$ 为电磁转矩（N·m），$K_t$ 的单位为 N·m/A。对于理想永磁电机，$K_t$ 与**反电动势常数（back-EMF constant）** $K_e$ 在 SI 单位制下数值相等（$K_t \approx K_e$），后者满足：

$$
E_b = K_e \cdot \omega_m
$$

$E_b$ 为线反电动势幅值（V），$\omega_m$ 为电机机械角速度（rad/s）。相电压平衡方程可写为：

$$
V_{dc} \approx \sqrt{3} \left( E_b + I \cdot R_s \right) = \sqrt{3} \left( K_e \omega_m + I R_s \right)
$$

$V_{dc}$ 为直流母线电压（V），$R_s$ 为相绕组电阻（Ω）。该式表明：在固定母线电压下，转速越高，可用于驱动电流的电压余量越小，从而限制了高速区的输出转矩——即**弱磁区（flux-weakening region）**的开始。

!!! note "术语解释：转矩常数、反电动势常数、相绕组电阻、弱磁区"
    - **转矩常数（$K_t$）**：单位电流产生的电磁转矩，N·m/A。
    - **反电动势常数（$K_e$）**：单位转速产生的感应电动势，V/(rad/s)。
    - **相绕组电阻（$R_s$）**：电机定子每相绕组的直流电阻，Ω。
    - **弱磁区**：通过施加直轴去磁电流扩展电机最高转速的运行区域。

电机损耗主要包括**铜损（copper loss）**、**铁损（iron loss）**与**机械损耗（mechanical loss）**。在关节伺服瞬态与短时峰值工况中，铜损通常占主导：

$$
P_{cu} = 3 I^2 R_s
$$

总损耗 $P_{loss}$ 最终转化为热量，使绕组温度上升。绕组温升可用一阶热阻模型描述：

$$
\Delta T = T_w - T_a = P_{loss} \cdot R_{th}
$$

$\Delta T$ 为绕组对环境温升（K），$T_w$ 为绕组温度（℃），$T_a$ 为环境温度（℃），$R_{th}$ 为绕组到环境的等效热阻（K/W）。由此可导出**连续转矩（continuous torque）** $T_{cont}$：

$$
T_{cont} = K_t \sqrt{ \frac{T_{w,max} - T_a}{3 R_s R_{th}} }
$$

$T_{w,max}$ 为绝缘等级允许的最高绕组温度（如 F 级 155 ℃）。实际峰值转矩 $T_{peak}$ 受磁钢退磁、绕组绝缘与驱动器电流极限共同约束，通常由电机datasheet给出，不能简单地由热模型外推。

对于周期性负载，可用**等效有效转矩（RMS torque）**校核热极限：

$$
T_{RMS} = \sqrt{ \frac{1}{t_{cycle}} \int_0^{t_{cycle}} T^2(t) \, dt }
$$

若 $T_{RMS} \le T_{cont}$，则电机在长期运行中不会过热；若存在短时峰值，则需校核 $T_{peak}$ 与驱动器电流极限 $I_{max}$：

$$
T_{peak} = K_t \cdot I_{max}
$$

!!! note "术语解释：铜损、铁损、热阻、连续转矩、RMS 转矩、峰值转矩"
    - **铜损**：电流流经绕组电阻产生的焦耳热。
    - **铁损**：交变磁场在铁芯中产生的磁滞与涡流损耗。
    - **热阻（thermal resistance）**：热量传递路径上对热流的阻力，K/W。
    - **连续转矩**：额定温升下可持续输出的转矩。
    - **RMS 转矩**：周期性负载的等效发热转矩。
    - **峰值转矩**：短时允许输出的最大转矩。
...
```mermaid
flowchart LR
    A["直流母线 Vdc"] --> B["逆变器 PWM"]
    B --> C["三相电流 Iq"]
    C --> D["电磁转矩 Te=Kt·Iq"]
    D --> E["机械输出"]
    C --> F["铜损 3I²Rs"]
    F --> G["绕组温升 ΔT=Ploss·Rth"]
    G -->|反馈限制| D
    H["反电动势 Eb=Ke·ω"] -->|降低可用电压| B
```

**数值示例**：某无框力矩电机 $K_t = 0.35\ \text{N·m/A}$，$R_s = 0.25\ \Omega$，$R_{th} = 2.5\ \text{K/W}$，环境温度 $T_a = 40\ ^\circ\text{C}$，绝缘等级 F（$T_{w,max}=155\ ^\circ\text{C}$）。
...
若驱动器峰值电流 30 A，则短时峰值转矩约为 $0.35 \times 30 = 10.5\ \text{N·m}$。人形机器人髋/膝等重载关节通常需要 30–100 N·m 级关节输出，因此必须配合高减速比减速器使用[226][228]。
...
#### 7.3.2.1 减速器效率、刚度与反射惯量

减速器不仅改变转速-扭矩关系，还决定传动链的能量损失、动态刚度与等效惯量。设减速比为 $N = \omega_{in}/\omega_{out} = T_{out}/T_{in}$（忽略损失），传动效率为 $\eta$，则输出功率与输入功率关系为：
...
效率损失主要来源于齿轮啮合摩擦、轴承摩擦、搅油损失与密封阻力。对于谐波减速器，典型效率 $\eta$ 在 80%–95% 之间，随负载、转速、润滑脂温度和运转时间变化；行星减速器效率通常 90%–98%；摆线与 RV 减速器因多齿啮合，效率略低于行星但刚度更高。

!!! note "术语解释：传动效率、减速比、机械利益、啮合损失"
    - **传动效率（transmission efficiency）**：输出功率与输入功率之比，反映能量损失。
    - **减速比（gear ratio）**：输入转速与输出转速之比。
    - **机械利益（mechanical advantage）**：输出力/转矩与输入力/转矩之比。
    - **啮合损失（meshing loss）**：齿轮啮合面摩擦与变形导致的功率损失。

从控制角度看，减速器将负载惯量**反射（reflect）**到电机轴，其等效惯量为：

$$
J_{ref} = \frac{J_{load}}{N^2}
$$

$J_{load}$ 为负载端转动惯量（kg·m²）。高减速比可显著降低电机侧所需加速转矩，但也会降低关节输出转速。总等效惯量需计入减速器自身惯量 $J_{gear}$ 与电机转子惯量 $J_m$：

$$
J_{eq} = J_m + J_{gear} + \frac{J_{load}}{N^2}
$$

传动链的**扭转刚度（torsional stiffness）** $K_\theta$ 决定关节在负载作用下的弹性变形。若电机侧刚度为 $K_m$、减速器输出侧等效刚度为 $K_g$，二者串联后的等效刚度为：
...
对于人形机器人关节，背隙与扭转刚度共同影响位置环带宽和力控精度。高动态关节通常要求背隙 $< 1\ \text{arcmin}$、扭转刚度 $> 10^4\ \text{N·m/rad}$。

!!! note "术语解释：反射惯量、等效惯量、扭转刚度、背隙、位置环带宽"
    - **反射惯量**：负载惯量折算到电机轴的等效值。
    - **等效惯量**：电机轴侧总转动惯量，包括电机、减速器与负载反射分量。
    - **扭转刚度**：传动链抵抗扭转变形的能力，N·m/rad。
    - **背隙（backlash）**：换向空程角。
    - **位置环带宽**：位置闭环能够响应的正弦指令频率上限。

```mermaid
flowchart TD
    A["电机转矩 Tin"] --> B["减速器 N, η"]
    B --> C["输出转矩 Tout=η·N·Tin"]
    D["负载惯量 Jload"] -->|反射| E["Jref=Jload/N²"]
    F["扭转刚度 Kg"] --> G["输出变形 θ=Tout/Keq"]
    C --> H["关节输出轴"]
    E --> H
    G --> H
```

**数值示例**：某谐波减速器 $N = 100$，$\eta = 0.90$，负载惯量 $J_{load} = 0.05\ \text{kg·m}^2$，电机转子惯量 $J_m = 2 \times 10^{-4}\ \text{kg·m}^2$，减速器自身惯量 $J_{gear} = 1 \times 10^{-4}\ \text{kg·m}^2$。
...
可见电机与减速器惯量占主导。若电机输出 3 N·m，关节输出转矩为：

$$
T_{out} = 0.90 \times 100 \times 3 = 270\ \text{N·m}
$$

这说明选择减速比时需要在“高减速比以获取大输出转矩”与“低减速比以保持高输出转速”之间权衡[235]。
...
#### 7.3.4.1 机器人计算平台的算力、能效与延迟模型

人形机器人计算平台需要同时满足感知、规划、控制与 AI 推理的异构计算需求。评价计算平台不能只看峰值算力（TOPS），而应从**算力（throughput）**、**能效（energy efficiency）**与**延迟（latency）**三个维度综合评估。

!!! note "术语解释：TOPS、FLOPS、算术强度、内存带宽、Roofline 模型、利用率"
    - **TOPS**：每秒万亿次整数/定点运算，常用于衡量 NPU/AI 加速器峰值算力。
    - **FLOPS**：每秒浮点运算次数。
    - **算术强度（arithmetic intensity）**：单位内存访问量对应的运算次数，单位 op/byte。
    - **内存带宽（memory bandwidth）**：处理器与内存之间单位时间可传输的数据量，单位 GB/s。
    - **Roofline 模型**：用峰值算力和内存带宽两条上限描述实际性能瓶颈的模型。
    - **利用率（utilization）**：实际性能与峰值性能的比值。

Roofline 模型把实际可达算力 $P$ 表示为峰值算力 $P_{peak}$ 与“内存带宽限制算力” $I \cdot B$ 二者的较小值：

$$
P = \min\left(P_{peak},\ I \cdot B\right)
$$

其中 $I$ 为算术强度（operations/byte），$B$ 为内存带宽（byte/s）。当 $I < P_{peak}/B$ 时，应用受内存带宽约束；当 $I > P_{peak}/B$ 时，受峰值算力约束。这个临界值称为**Ridge point**：
...
```mermaid
flowchart LR
    A["应用算法<br/>算术强度 I"] --> B{"I < I_ridge ?"}
    B -->|是| C["内存带宽受限<br/>P = I·B"]
    B -->|否| D["峰值算力受限<br/>P = P_peak"]
    C --> E["优化方向：<br/>提高数据复用/缓存命中率"]
    D --> F["优化方向：<br/>提高并行效率/降低静态功耗"]
```

能效常用 **TOPS/W** 表示。若功耗分为静态功耗 $P_{static}$ 和与算力成正比的动态功耗 $P_{dynamic}=\alpha P$（$\alpha$ 为能耗系数，单位 J/op），则：

$$
E = \frac{P}{P_{static} + \alpha P}
$$

当 $P$ 接近峰值时，能效趋近于 $1/\alpha$；低负载时静态功耗占比上升，能效下降。

**数值示例**：某机器人 SoC 的 NPU 峰值算力 $P_{peak}=100\ \text{TOPS}$，内存带宽 $B=100\ \text{GB/s}$，静态功耗 $P_{static}=5\ \text{W}$，每 TOPS 动态能耗 $\alpha=0.1\ \text{W/TOPS}$。
...
若某感知网络算术强度 $I=500\ \text{op/byte}$，则实际算力受带宽限制：

$$
P = 500 \times 100 \times 10^9 = 50\ \text{TOPS}
$$

对应功耗 $P_{total}=5 + 0.1 \times 50 = 10\ \text{W}$，能效为 $50/10=5\ \text{TOPS/W}$。若算法优化使 $I$ 提升到 $2000\ \text{op/byte}$，则算力达到峰值 $100\ \text{TOPS}$，功耗 $15\ \text{W}$，能效 $6.67\ \text{TOPS/W}$。这说明：在带宽受限区提升算法数据复用率比单纯提高峰值算力更有效。

延迟方面，若网络每层需要处理的数据量为 $D$（byte），带宽为 $B$，则仅数据搬运时间就至少需要：

$$
T_{memory} = \frac{D}{B}
$$

若总计算量为 $W$（ops），实际算力为 $P$，则计算时间为 $T_{compute}=W/P$。端到端延迟通常由二者中较大者决定。控制回路对人形机器人要求毫秒级延迟，因此除峰值算力外，**确定性延迟**和 **worst-case execution time（WCET）** 也是关键指标。详情见第 6 章 6.3 节。
...
!!! note "术语解释：确定性延迟、WCET、内存墙、计算强度"
    - **确定性延迟（deterministic latency）**：可严格保证的最大响应时间。
    - **WCET（Worst-Case Execution Time）**：最坏情况执行时间。
    - **内存墙（memory wall）**：处理器算力增长远快于内存带宽，导致实际性能受限于数据搬运。
    - **计算强度提升**：通过复用片上数据、增加缓存命中率来提高算术强度。
...
#### 7.3.5.1 锂离子电池等效电路与能量-功率特性

锂离子电池可抽象为理想电压源 $V_{oc}(SoC)$ 与内阻 $R_{int}$ 串联的戴维南等效电路。端电压 $V_t$ 与电流 $I$ 的关系为：

$$
V_t = V_{oc}(SoC) - I \cdot R_{int}
$$

其中 $V_{oc}$ 为开路电压，随荷电状态 $SoC$ 变化；$R_{int}$ 为电池内阻，包含欧姆内阻与极化内阻。放电时 $I>0$，端电压下降；充电时 $I<0$，端电压上升。

!!! note "术语解释：开路电压、内阻、荷电状态、极化、戴维南等效"
    - **开路电压（OCV）**：电池在无负载时的端电压，与 SoC 和温度相关。
    - **内阻（internal resistance）**：电池内部对电流的阻力，导致电压降和发热。
    - **荷电状态（SoC）**：剩余容量与额定容量之比，0–100%。
    - **极化（polarization）**：电化学反应偏离平衡态引起的电压损失。
    - **戴维南等效（Thevenin equivalent）**：把复杂网络等效为电压源与串联电阻的电路模型。

```mermaid
flowchart LR
    A["V_oc(SoC)<br/>理想电压源"] --> B["R_int<br/>内阻"]
    B --> C["V_t<br/>端电压"]
    C --> D["负载 R_L"]
    D --> E["电流 I"]
    E --> F["发热 I²R_int"]
...
电池可输出的最大功率受端电压下限 $V_{min}$ 限制。令 $V_t=V_{min}$，则电流 $I=(V_{oc}-V_{min})/R_{int}$，于是：

$$
P_{max} = V_{min} \cdot I = \frac{(V_{oc} - V_{min}) V_{min}}{R_{int}}
$$

该式说明：内阻越小、开路电压越高，电池越能同时提供高能量与高功率。

能量密度 $E_g$（Wh/kg）与功率密度 $P_g$（W/kg）的 Ragone 关系可近似为：

$$
E_g \cdot P_g \le \frac{V_{oc}^2}{4 R_{int} \cdot m_{cell}}
$$

其中 $m_{cell}$ 为电芯质量。该不等式表明能量密度与功率密度之间存在基本权衡：高功率需要低内阻设计，可能牺牲活性物质比例从而降低能量密度。

C-rate 描述充放电电流相对于额定容量 $C_{nom}$（Ah）的倍数：

$$
I = C_{rate} \cdot C_{nom}
$$

1C 表示 1 小时放完额定容量。人形机器人瞬时功率需求可能达到 2–5C，但持续高倍率放电会加速老化并增加热负荷。锂、钴、镍等电池关键材料的资源分布与供应链风险详见第 3 章。

**数值示例**：某 NCM 电芯额定容量 $C_{nom}=5\ \text{Ah}$，平均开路电压 $V_{oc}=3.7\ \text{V}$，内阻 $R_{int}=10\ \text{m}\Omega$，质量 $m=0.20\ \text{kg}$，端电压下限 $V_{min}=3.0\ \text{V}$。
...
此时发热功率占输出功率 23%，热管理成为高倍率放电的关键约束。详情见第 6 章 6.2 节。
...
print(f"能量密度 = {E_g:.1f} Wh/kg")
print(f"最大功率 = {P_max:.1f} W")
print(f"功率密度 = {P_g:.1f} W/kg")
print(f"最大电流 = {I_max:.1f} A ({I_max/C_nom:.1f}C)")

# Ragone 曲线：改变内阻观察能量-功率权衡
R_values = np.array([0.005, 0.010, 0.020, 0.030])  # ohm
E_g_const = E_g  # 假设能量密度不变
...
!!! note "术语解释：Ragone 曲线、C-rate、功率密度、能量密度、热负荷"
    - **Ragone 曲线**：能量密度与功率密度之间的权衡曲线。
    - **C-rate**：相对于额定容量的充放电倍率。
    - **功率密度**：单位质量可输出的功率。
    - **能量密度**：单位质量存储的能量。
    - **热负荷**：高倍率放电产生的焦耳热。

#### 7.3.5.2 功率半导体开关损耗与结温模型

功率半导体（MOSFET/IGBT/SiC/GaN）在逆变器中承担高频开关任务。损耗分为**导通损耗（conduction loss）**和**开关损耗（switching loss）**。导通损耗由导通电阻 $R_{ds(on)}$ 或饱和压降 $V_{ce(sat)}$ 产生；开关损耗与开关频率 $f_{sw}$、直流母线电压 $V_{dc}$、负载电流 $I$ 成正比。

对于三相逆变器中的一相桥臂，采用 PWM 控制时，MOSFET 的导通损耗可近似为：

$$
P_{cond} = I_{rms}^2 \cdot R_{ds(on)} \cdot D
$$

其中 $I_{rms}$ 为漏极电流有效值，$D$ 为占空比。开关损耗经验模型为：

$$
P_{sw} = \frac{1}{2} V_{dc} \cdot I \cdot (t_{on} + t_{off}) \cdot f_{sw}
$$

$t_{on}$、$t_{off}$ 为开通/关断时间。总损耗：

$$
P_{loss} = P_{cond} + P_{sw}
$$

!!! note "术语解释：导通损耗、开关损耗、占空比、开关频率、结温、热阻"
    - **导通损耗**：器件导通时由于电阻/压降产生的损耗。
    - **开关损耗**：器件在开通和关断过程中由于电压电流交叠产生的损耗。
    - **占空比（duty cycle）**：PWM 周期内导通时间所占比例。
    - **开关频率**：单位时间内开关次数。
    - **结温（junction temperature）**：半导体 PN 结温度。
    - **热阻（thermal resistance）**：热量从结到散热环境的路径阻力。

```mermaid
flowchart TD
    A["逆变器总损耗 P_loss"] --> B["导通损耗<br/>I_rms²·R_ds(on)·D"]
    A --> C["开关损耗<br/>½V_dc·I·(t_on+t_off)·f_sw"]
    B --> D["结温升<br/>T_j = T_a + P_loss·R_th"]
    C --> D
    D --> E["热设计约束<br/>T_j ≤ T_j,max"]
```
...
$T_a$ 为环境温度，$R_{th,jc}$ 为结到壳热阻，$R_{th,ch}$ 为壳到散热器热阻，$R_{th,ha}$ 为散热器到环境热阻。SiC/GaN 因导通电阻低、开关速度快，可显著降低 $P_{loss}$，从而减少散热体积。

**数值示例**：某 SiC MOSFET 的 $R_{ds(on)}=5\ \text{m}\Omega$，$t_{on}+t_{off}=50\ \text{ns}$，驱动关节电流有效值 $I_{rms}=10\ \text{A}$，占空比 $D=0.5$，$V_{dc}=48\ \text{V}$，$f_{sw}=20\ \text{kHz}$，总热阻 $R_{th}=2.0\ \text{K/W}$，$T_a=40\ ^\circ\text{C}$。
...
作为对比，若用硅基 MOSFET（$R_{ds(on)}=30\ \text{m}\Omega$，$t_{on}+t_{off}=200\ \text{ns}$）：
...
总损耗 $2.46\ \text{W}$，结温 $44.9\ ^\circ\text{C}$。虽然单个器件温升不大，但整机有数十个关节驱动器，SiC 可使总热耗降低约 5 倍，显著减小散热器和电池负担。
...
!!! note "术语解释：SiC MOSFET、GaN HEMT、导通电阻、开关时间、热阻链"
    - **SiC MOSFET**：碳化硅金属氧化物半导体场效应晶体管。
    - **GaN HEMT**：氮化镓高电子迁移率晶体管。
    - **导通电阻**：MOSFET 完全导通时漏源极之间的电阻。
    - **开关时间**：器件从关断到导通或从导通到关断所需时间。
    - **热阻链**：从半导体结到环境的串联热阻路径。
...
#### 7.3.7.16 关节模组物理建模与选型公式

关节模组可抽象为“电机 + 减速器 + 传感器 + 结构”的集成系统。其输出端转矩、转速、惯量与热特性共同决定机器人能否完成目标运动。设电机电磁转矩为 $T_m$，减速比为 $N$，效率为 $\eta$，则关节输出转矩 $T_j$ 与输出转速 $\omega_j$ 分别为：
...
其中 $\omega_m$ 为电机机械转速（rad/s）。若直流母线电压为 $V_{dc}$，电机反电动势常数为 $K_e$，相电阻为 $R_s$，则电机最大空载转速近似为：

$$
\omega_{m,max} \approx \frac{V_{dc}/\sqrt{3} - I_{max} R_s}{K_e}
$$

对应关节最大输出转速为 $\omega_{j,max} = \omega_{m,max}/N$。

从动力学角度，关节输出轴侧总惯量 $J_{out}$ 与电机侧等效惯量 $J_{eq}$ 的关系为：
...
其中 $\dot{\omega}_j$ 为关节输出角加速度（rad/s²）。总电机转矩需求为负载转矩、摩擦转矩与加速转矩之和：

$$
T_m = \frac{T_{load}}{\eta N} + T_{friction} + J_{eq} \cdot \dot{\omega}_m
$$

热约束方面，若关节运动周期为 $t_{cycle}$，转矩曲线为 $T_m(t)$，则需满足：
...
!!! note "术语解释：关节输出转矩、等效惯量、RMS 校核、峰值校核、减速比选型"
    - **关节输出转矩**：关节输出轴可提供的转矩，N·m。
    - **等效惯量**：折算到电机轴的总惯量。
    - **RMS 校核**：用等效发热转矩验证电机长期热极限。
    - **峰值校核**：验证电机/驱动器短时最大转矩能力。
    - **减速比选型**：在满足速度与转矩要求的前提下，使电机工作在高效区并最小化惯量失配。

**减速比选型的物理直觉**：增大 $N$ 可以降低电机转矩需求和反射惯量，但会降低输出转速并放大减速器背隙、摩擦与惯量的影响；减小 $N$ 则要求电机高转速、高转矩，可能增加电机体积与成本。工程上常以**惯量匹配系数** $M = J_{eq}/J_m$ 作为经验指标，伺服关节通常希望 $M$ 在 1–10 之间[226][227]。

```mermaid
flowchart TD
    A["目标关节运动<br/>Tj(t), ωj(t)"] --> B["减速比 N"]
    B --> C["电机转速/转矩<br/>ωm=N·ωj, Tm=Tj/(ηN)"]
    C --> D{"RMS ≤ Tcont?"}
    D -->|否| E["增大电机或降低 N"]
    E --> B
    D -->|是| F{"峰值 ≤ Tpeak?"}
...
#### 7.3.7.17 Python 算例：关节模组热约束与峰值扭矩校核

下面的代码根据目标关节运动曲线，计算电机侧等效 RMS 转矩与峰值转矩，并判断是否满足连续/峰值限制。
...
print(f"电机连续转矩 T_cont = {T_cont:.2f} N·m")
print(f"电机峰值转矩 T_peak = {T_peak:.2f} N·m")

# 模拟一个步态周期内的关节转矩与速度 (示例数据)
t = np.linspace(0, 1.0, 1000)  # 1 s 周期
omega_j = 2.0 * np.sin(2 * np.pi * t)          # rad/s
alpha_j = np.gradient(omega_j, t)
T_load = 15.0 * np.sin(2 * np.pi * t + 0.5)    # N·m 输出端负载
T_friction = 0.05                               # N·m 电机侧摩擦
...
print(f"\n关节输出负载峰值: {np.max(np.abs(T_load)):.2f} N·m")
print(f"电机侧 RMS 转矩: {T_rms:.2f} N·m")
print(f"电机侧峰值转矩: {T_peak_req:.2f} N·m")
print(f"RMS 校核: {'通过' if T_rms <= T_cont else '不通过'}")
print(f"峰值校核: {'通过' if T_peak_req <= T_peak else '不通过'}")
...
!!! note "术语解释：热约束、峰值校核、运动周期、惯量匹配"
    - **热约束**：电机长期运行时绕组温升不超过绝缘极限。
    - **峰值校核**：短时最大转矩不超过电机磁路与驱动器电流极限。
    - **运动周期**：重复性运动的一个完整循环时间。
    - **惯量匹配**：电机惯量与负载反射惯量的比例关系。
...
#### 7.5.4.1 TTR/TTS 驱动的安全库存与多源决策模型

TTR 与 TTS 的对比可量化供应链韧性缺口。设某关键物料的日均需求为 $d$（件/天），当前库存为 $I_0$，已下单在途库存为 $I_{transit}$，合格备选供应商切换后可在 $t_{alt}$ 天内交付第一批货，则总可用供应当量为 $I_0 + I_{transit} + d \cdot t_{alt}$，于是：

$$
TTS = \frac{I_0 + I_{transit} + d \cdot t_{alt}}{d}
$$

若企业希望 $TTS \ge TTR$，所需最低库存水平为：

$$
I_{target} = d \cdot (TTR - t_{alt}) - I_{transit}
$$

当 $t_{alt}=0$（无备选、只能消耗库存）时，$I_{target}=d \cdot TTR$，即需要覆盖整个恢复周期的库存。发展备选供应商相当于用 $t_{alt}$ 抵扣部分库存需求。安全库存与 EOQ 的定量模型见第 7 章 7.6.3 节。

!!! note "术语解释：在途库存、安全库存目标、恢复周期、备选切换时间"
    - **在途库存（pipeline inventory）**：已下单但尚未到货的物料。
    - **安全库存目标（safety stock target）**：为满足服务水平所需的库存水位。
    - **恢复周期**：供应商从断供恢复到正常交付所需时间。
    - **备选切换时间**：从中断发生到备选供应商首批交付所需时间。

```mermaid
flowchart TD
    A["供应中断"] --> B["评估 TTR"]
    C["当前库存+在途+备选<br/>可用供应当量"] --> D["计算 TTS"]
    B --> E{"TTS ≥ TTR ?"}
    D --> E
    E -->|是| F["维持生产<br/>启动恢复跟踪"]
    E -->|否| G["缺口 = TTR - TTS"]
    G --> H["提高安全库存"]
    G --> I["启用/发展备选供应商"]
    G --> J["缩短采购提前期"]
...
引入多源采购后，若主供份额为 $\alpha$、备选份额为 $1-\alpha$，且两源独立断供概率分别为 $p_1$、$p_2$，则该物料月度断供期望损失可近似为：

$$
E[\text{loss}] = \alpha \cdot p_1 + (1-\alpha) \cdot p_2 - \alpha(1-\alpha) \cdot p_1 p_2 \cdot \beta
$$

其中 $\beta$ 为双源同时失效时的部分补偿系数（$0 \le \beta \le 1$）。第三项表示两源同时断供时，企业仍可通过库存、内部产能或市场采购弥补部分缺口，因此实际损失小于两源损失之和。

**数值示例**：某谐波减速器日均需求 $d=20$ 件，主供 TTR=60 天，备选供应商可在 $t_{alt}=10$ 天内启动交付，在途库存 $I_{transit}=200$ 件。

无备选库存目标：

$$
I_{target} = 20 \times (60 - 0) - 200 = 1000\ \text{件}
$$

有备选库存目标：

$$
I_{target} = 20 \times (60 - 10) - 200 = 800\ \text{件}
$$

即引入备选供应商可减少 200 件安全库存，按单价 2000 元/件、年持有成本率 20% 计算，年节省库存资金成本：

$$
200 \times 2000 \times 0.20 = 80{,}000\ \text{元}
$$

但维持备选供应商需要样品验证、小批量采购和质量审核，年固定成本约 15 万元。因此只有当断供概率或停产损失足够高时，双源才降低总成本。
...
d = 20                 # 日均需求（件）
TTR = 60               # 恢复时间（天）
unit_price = 2000      # 单价（元）
holding_rate = 0.20    # 年持有成本率
alt_setup_cost = 150_000  # 备选供应商年固定成本（元）

I_transit = 200
t_alt_range = np.linspace(0, 30, 31)  # 备选启动时间 0-30 天

I_target = np.maximum(0, d * (TTR - t_alt_range) - I_transit)
annual_holding_cost = I_target * unit_price * holding_rate
# 只要启用备选（t_alt < 30 视为启用）就承担固定成本
total_cost = annual_holding_cost + alt_setup_cost * (t_alt_range < 30)

plt.figure(figsize=(8, 4))
plt.plot(t_alt_range, annual_holding_cost/1e4, label="库存持有成本")
plt.plot(t_alt_range, total_cost/1e4, label="总成本（含备选固定成本）")
plt.axhline(d*TTR*unit_price*holding_rate/1e4, color="red", linestyle="--", label="无备选库存成本")
plt.xlabel("备选供应商启动时间 t_alt (天)")
plt.ylabel("成本（万元/年）")
plt.title("TTR/TTS 驱动的库存-多源成本权衡")
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.show()
...
for t_alt in [0, 10, 20, 30]:
    ...
    print(f"t_alt={t_alt:2d}天: 目标库存={inv:.0f}件, 年持有成本={cost/1e4:.2f}万元")
```

!!! note "术语解释：期望损失、多源份额、补偿系数、库存资金成本"
    - **期望损失（expected loss）**：风险事件发生损失的加权平均值。
    - **多源份额**：采购量在不同供应商之间的分配比例。
    - **补偿系数**：多源同时失效时企业通过其他手段弥补缺口的比例。
    - **库存资金成本**：库存占用资金的机会成本。
...
供应链本质上是一个由供应商、工厂、物流节点和客户构成的**有向网络（directed network）**。单个节点的失效会通过供应关系向上游或下游传播，造成级联失效（cascading failure）。理解这种网络特性需要借助图论与网络科学工具[230][231][232]。

设供应链网络由 $n$ 个节点组成，用**邻接矩阵（adjacency matrix）** $\mathbf{A} \in \{0,1\}^{n \times n}$ 描述供应关系：若节点 $i$ 向节点 $j$ 供应物料，则 $A_{ij} = 1$，否则为 0。节点 $i$ 的**入度（in-degree）** $k_i^{in}$ 与**出度（out-degree）** $k_i^{out}$ 分别表示其上游供应商数量与下游客户数量：

$$
k_i^{in} = \sum_j A_{ji}, \quad k_i^{out} = \sum_j A_{ij}
$$

!!! note "术语解释：邻接矩阵、入度、出度、有向网络、供应关系"
    - **邻接矩阵**：描述网络中节点之间连接关系的矩阵。
    - **入度**：指向某节点的边数，代表上游供应源数量。
    - **出度**：从某节点出发的边数，代表下游客户数量。
    - **有向网络**：边具有方向的图网络。
    - **供应关系**：供应商向买方提供产品或服务的连接。

节点的**脆弱性（vulnerability）**不仅取决于自身失效概率，还取决于它在网络中的位置。**介数中心性（betweenness centrality）**衡量节点作为“桥梁”的重要性：若大量最短供应路径经过某节点，则其中断将造成广泛影响。

$$
B_i = \sum_{s \neq i \neq t} \frac{\sigma_{st}(i)}{\sigma_{st}}
$$

其中 $\sigma_{st}$ 为节点 $s$ 到 $t$ 的最短路径总数，$\sigma_{st}(i)$ 为经过节点 $i$ 的最短路径数。

级联失效可用**阈值模型（threshold model）**近似：节点 $i$ 失效后，其下游节点 $j$ 若失去的供应输入比例超过其冗余阈值 $\theta_j$，则 $j$ 也失效。形式上，若节点 $j$ 有 $k_j^{in}$ 个上游节点，其中 $f_j$ 个失效，则当：

$$
\frac{f_j}{k_j^{in}} \ge \theta_j
$$

时，节点 $j$ 进入失效状态。该过程迭代进行，直到没有新节点失效为止。对于单源供应的关键节点，$\theta_j \approx 0$，极易发生级联；对于多源或持有安全库存的节点，$\theta_j$ 较高，韧性更强。

!!! note "术语解释：介数中心性、级联失效、阈值模型、冗余阈值、网络韧性"
    - **介数中心性**：衡量节点在最短路径中充当桥梁程度的指标。
    - **级联失效**：一个节点失效引发相邻节点连续失效的传播过程。
    - **阈值模型**：节点是否失效取决于受损输入比例是否超过阈值的简化模型。
    - **冗余阈值**：节点能够承受的失效上游比例上限。
    - **网络韧性**：网络在扰动后维持连通与功能的能力。
...
**网络脆弱性改进指标**：除介数中心性外，还可计算**网络连通度（connectivity）**、**平均路径长度（average path length）**与**最大连通子图大小（giant connected component size）**作为韧性度量。多源采购、安全库存与替代路径本质上都是在降低网络对关键节点的依赖。

#### 7.5.9 Python 算例：供应链网络级联失效模拟

下面的代码用 `networkx` 构建一个简化的人形机器人供应链网络，计算节点度与介数中心性，并模拟随机攻击与针对性攻击下的级联失效。
...
edges = [
    ("稀土矿", "磁材厂"), ("磁材厂", "无框电机厂"),
    ("锂矿", "正极材料厂"), ("正极材料厂", "电芯厂"),
    ("铜矿", "铜材厂"), ("铜材厂", "无框电机厂"), ("铜材厂", "结构件厂"),
    ("无框电机厂", "执行器模组"), ("减速器厂", "执行器模组"),
    ("驱动器厂", "执行器模组"), ("结构件厂", "执行器模组"),
    ("电芯厂", "电池包"), ("执行器模组", "OEM"), ("电池包", "OEM"),
]
G.add_edges_from(edges)
...
# 随机攻击：随机失效 2 个上游节点
np.random.seed(3)
random_fail = set(np.random.choice([n for n in nodes if nodes[n][0] == "上游"], size=2, replace=False))
failed_random = cascading_failure(G, random_fail, threshold=0.5)
print(f"\n随机攻击 {random_fail} 导致失效节点数: {len(failed_random)}")

# 针对性攻击：失效介数中心性最高的 2 个节点
...
print(f"针对性攻击 {set(top_nodes)} 导致失效节点数: {len(failed_targeted)}")
...
!!! note "术语解释：随机攻击、针对性攻击、介数中心性攻击、级联范围"
    - **随机攻击**：随机选择节点使其失效。
    - **针对性攻击**：优先攻击网络中重要性高的节点。
    - **介数中心性攻击**：按介数中心性排序进行针对性攻击。
    - **级联范围**：一次初始失效最终导致的总失效节点数。
...
#### 7.8.2.1 人形机器人碳足迹简化核算模型

供应链碳足迹核算可从零部件层面自下而上汇总。对于某零部件 $i$，其碳排放量 $E_i$ 等于活动数据 $AD_i$（材料质量、能耗、运输吨公里等）乘以排放因子 $EF_i$：
...
排放因子通常引用 IPCC、生态环境部或行业数据库（如 Ecoinvent、GaBi）。范围 1/2/3 的划分本质是排放责任边界：范围 1 是企业直接燃烧或工艺排放，范围 2 是外购电力热力，范围 3 是上游材料与下游使用/报废。范围 1/2/3 的详细定义见第 7 章 7.8.2 节。

!!! note "术语解释：活动数据、排放因子、排放责任边界、碳当量、GWP"
    - **活动数据（activity data）**：导致排放的活动量，如 kg 材料、kWh 电、ton·km 运输。
    - **排放因子（emission factor）**：单位活动数据对应的二氧化碳当量排放。
    - **排放责任边界（emission boundary）**：企业核算碳排放时包含的范围。
    - **碳当量（CO₂e）**：把不同温室气体按全球变暖潜势换算为二氧化碳当量。
    - **GWP（Global Warming Potential）**：某种温室气体在特定时间尺度内相对于 CO₂ 的变暖能力。

```mermaid
flowchart TD
    A["产品碳足迹 E_product"] --> B["范围1<br/>直接排放"]
    A --> C["范围2<br/>外购电力/热力"]
    A --> D["范围3<br/>上游材料与下游使用/报废"]
    B --> B1["工厂燃烧/工艺过程"]
    C --> C1["电力采购"]
    D --> D1["稀土/磁材/电池"]
    D --> D2["运输与物流"]
...
对于人形机器人旋转关节模组，主要排放来源包括：

1. 铝合金壳体：$m_{Al}$（kg）× $EF_{Al}$（约 8–12 kg CO₂e/kg）。
2. 铜绕组：$m_{Cu}$（kg）× $EF_{Cu}$（约 2–4 kg CO₂e/kg）。
3. 钕铁硼磁体：$m_{NdFeB}$（kg）× $EF_{NdFeB}$（约 30–60 kg CO₂e/kg，取决于稀土冶炼电力结构）。
4. 电池：$E_{cell}$（Wh）× $EF_{cell}$（约 60–150 kg CO₂e/kWh）。
5. 运输：质量 × 距离 × 运输排放因子。

**数值示例**：一个关节模组的质量与能耗假设如下：

| 物料/活动 | 活动数据 | 排放因子 | 碳排放（kg CO₂e） |
|---|---:|---:|---:|
| 铝合金壳体 | 0.35 kg | 10 kg CO₂e/kg | 3.50 |
| 铜绕组 | 0.20 kg | 3 kg CO₂e/kg | 0.60 |
| 钕铁硼磁体 | 0.05 kg | 45 kg CO₂e/kg | 2.25 |
| 电池（整机分摊） | 100 Wh | 100 kg CO₂e/kWh | 10.00 |
| 机加工耗电 | 0.5 kWh | 0.55 kg CO₂e/kWh | 0.28 |
| 运输（500 km 卡车） | 0.6 kg × 500 km | 0.0001 kg CO₂e/(kg·km) | 0.03 |
...
若整机含 40 个关节，且电池整机排放为 400 kg CO₂e，则整机生产阶段范围 3 排放约为：

$$
E_{embodied} = 40 \times 16.66 + 400 \approx 1{,}066\ \text{kg CO₂e}
$$

这相当于一辆普通燃油车行驶约 5 000–6 000 km 的尾气排放。需要说明的是，排放因子随电网清洁度、回收材料比例和工艺效率变化显著，上述数值仅用于说明核算方法。

```python
import pandas as pd

data = {
    "项目": ["铝合金壳体", "铜绕组", "钕铁硼磁体", "电池（整机分摊）", "机加工耗电", "运输"],
    "活动数据": [0.35, 0.20, 0.05, 0.100, 0.5, 0.6*500],
    "单位": ["kg", "kg", "kg", "kWh", "kWh", "kg·km"],
    "排放因子": [10.0, 3.0, 45.0, 100.0, 0.55, 0.0001],
    "EF单位": ["kgCO2e/kg", "kgCO2e/kg", "kgCO2e/kg", "kgCO2e/kWh", "kgCO2e/kWh", "kgCO2e/(kg·km)"],
}
df = pd.DataFrame(data)
df["碳排放(kgCO2e)"] = df["活动数据"] * df["排放因子"]

print(df.to_string(index=False))
print(f"\n单个关节模组碳足迹 = {df['碳排放(kgCO2e)'].sum():.2f} kg CO2e")

n_joints = 40
battery_embodied = 400
total = n_joints * df["碳排放(kgCO2e)"].sum() + battery_embodied
print(f"整机（含{n_joints}个关节）生产阶段范围3排放 ≈ {total:.0f} kg CO2e")
```

!!! note "术语解释：范围3排放、隐含碳、电网排放因子、回收材料比例"
    - **范围3排放**：企业价值链上下游产生的间接排放。
    - **隐含碳（embodied carbon）**：产品制造过程中蕴藏的碳排放。
    - **电网排放因子**：单位电力消费对应的二氧化碳排放。
    - **回收材料比例**：产品中使用再生材料的比例，可显著降低隐含碳。
...
#### 7.9.6 制造工序 should-cost 分解实例

should-cost 不仅是一个总价，更是一套按工序、按资源逐项拆解的工程估算。以人形机器人关节铝合金壳体为例，其 should-cost 至少包括：**材料成本**、**压铸成本**、**CNC 精加工成本**、**表面处理成本**、**模具/刀具摊销**与**良率损失**[229][233][234]。通用表达式为：

$$
C_{should} = \sum_k \frac{C_{material,k}}{Y_k} + \sum_k t_k \cdot R_k + \sum_k \frac{C_{tool,k}}{N_{life,k}} + C_{overhead} + C_{profit}
$$

其中 $Y_k$ 为工序 $k$ 的良率（yield），$t_k$ 为单件加工时间（h），$R_k$ 为设备/人工小时费率（元/h），$C_{tool,k}$ 为刀具或模具成本，$N_{life,k}$ 为寿命件数，$C_{overhead}$ 为制造费用与管理费用分摊，$C_{profit}$ 为供应商合理利润。

!!! note "术语解释：工序成本、设备费率、刀具摊销、良率、制造费用"
    - **工序成本**：完成一道加工或装配工序所需的直接成本。
    - **设备费率（machine rate）**：设备每小时折旧、能源、维护与操作成本之和。
    - **刀具摊销**：把刀具或模具成本分摊到每件产品上的费用。
    - **良率（yield）**：合格品数量占总投入数量的比例。
    - **制造费用（overhead）**：间接生产费用，如厂房折旧、质量、物流、管理。

**材料成本**首先取决于毛坯重量、材料单价与材料利用率。压铸壳体通常按毛坯重量报价，CNC 后去除材料形成切屑：

$$
C_{material} = \frac{W_{blank} \cdot P_{mat}}{Y_{blank}}
$$

$W_{blank}$ 为毛坯重量（kg），$P_{mat}$ 为铝合金锭单价（元/kg），$Y_{blank}$ 为毛坯良品率。

**压铸成本**与压铸机吨位、节拍时间、人工与能源费率相关：

$$
C_{die\_cast} = t_{cycle} \cdot R_{die\_cast} + \frac{C_{die}}{N_{die\_life}}
$$

$t_{cycle}$ 为压铸节拍（h/件），$R_{die\_cast}$ 为压铸机综合费率（元/h），$C_{die}$ 为模具投资，$N_{die\_life}$ 为模具寿命件数。
...
$i$ 遍历各加工面，$j$ 遍历刀具。关键配合面（轴承位、密封面、螺纹底孔）需要精加工，刀具磨损快，应单独核算。

**表面处理成本**（阳极氧化、喷涂等）通常按面积或重量计费：

$$
C_{surface} = A_{surface} \cdot P_{surface}
$$

$A_{surface}$ 为处理面积（m²），$P_{surface}$ 为单位面积价格（元/m²）。

!!! note "术语解释：压铸节拍、模具寿命、CNC 工时、表面处理面积、材料利用率"
    - **压铸节拍**：完成一次压铸循环所需时间。
    - **模具寿命**：模具在报废前可生产的合格件数。
    - **CNC 工时**：数控机床加工单件所需时间。
    - **表面处理面积**：需要阳极氧化、喷涂等处理的表面积。
    - **材料利用率**：成品重量占毛坯重量的比例。

```mermaid
flowchart TD
    A["should-cost 分解"] --> B["材料成本"]
    A --> C["压铸成本"]
    A --> D["CNC 成本"]
    A --> E["表面处理成本"]
    A --> F["模具/刀具摊销"]
...
#### 7.9.7 Python 算例：关节壳体 should-cost 计算器
...
costs = model.cost()
print(f"年产量 {model.N:,.0f} 件时的 should-cost 分解（元/件）：")
...
!!! note "术语解释：should-cost 计算器、批量敏感性、模具摊销、间接费用率"
    - **should-cost 计算器**：按工程和经济假设逐项估算产品成本的工具。
    - **批量敏感性**：成本随年产量变化的分析。
    - **模具摊销**：把模具固定投资分摊到每件产品的成本。
    - **间接费用率**：间接费用占直接成本的比例。
...
| 符号 | 含义 | 常用单位 | 首次出现章节 |
|---|---|---|---|
| $C_{\text{BOM}}$ | 物料清单滚动成本 | 元 | 7.2.1 |
| $q_i$ | 第 $i$ 个零件单位用量 | 件/台 | 7.2.1 |
| $p_i$ | 第 $i$ 个零件单价 | 元/件 | 7.2.1 |
| $K_t$ | 电机转矩常数 | N·m/A | 7.3.1.1 |
| $K_e$ | 反电动势常数 | V/(rad/s) | 7.3.1.1 |
| $R_s$ | 相绕组电阻 | Ω | 7.3.1.1 |
| $R_{th}$ | 热阻 | K/W | 7.3.1.1 |
| $N$ | 减速比 | 无量纲 | 7.3.2.1 |
| $\eta$ | 传动效率 | 无量纲 | 7.3.2.1 |
| $J$ | 转动惯量 | kg·m² | 7.3.2.1 |
| $K_\theta$ | 扭转刚度 | N·m/rad | 7.3.2.1 |
| $P_{peak}$ | 峰值算力 | TOPS | 7.3.4.1 |
| $B$ | 内存带宽 | GB/s | 7.3.4.1 |
| $I$ | 算术强度 | op/byte | 7.3.4.1 |
| $V_{oc}$ | 电池开路电压 | V | 7.3.5.1 |
| $R_{int}$ | 电池内阻 | Ω | 7.3.5.1 |
| $SoC$ | 荷电状态 | % | 7.3.5.1 |
| $C_{rate}$ | 充放电倍率 | 1/h | 7.3.5.1 |
| $R_{ds(on)}$ | MOSFET 导通电阻 | Ω | 7.3.5.2 |
| $f_{sw}$ | 开关频率 | Hz | 7.3.5.2 |
| $T_j$ | 结温 | °C | 7.3.5.2 |
| CR3/CR5 | 市场集中度 | % | 7.5.2 |
| HHI | 赫芬达尔-赫希曼指数 | 0–10 000 | 7.5.2 |
| RPN | 风险优先数 | 1–1000 | 7.5.3 |
| TTR | 时间到恢复 | 天 | 7.5.4 |
| TTS | 时间到耗尽 | 天 | 7.5.4 |
| $d$ | 日均需求 | 件/天 | 7.5.4.1 |
| $SS$ | 安全库存 | 件 | 7.6.3 |
| $ROP$ | 再订货点 | 件 | 7.6.3 |
| $Q^*$ | 经济订货批量 | 件 | 7.6.3 |
| $C_{should}$ | should-cost 估算成本 | 元 | 7.9.1 |
| $LR$ | 学习率 | % | 7.9.2 |
| $TCO$ | 总拥有成本 | 元 | 7.9.4 |
| $E_i$ | 零部件碳排放 | kg CO₂e | 7.8.2.1 |
| $EF_i$ | 排放因子 | kg CO₂e/单位 | 7.8.2.1 |
| $AD_i$ | 活动数据 | varies | 7.8.2.1 |
| $LQ$ | 区位熵 | 无量纲 | 7.4.5 |

本章符号遵循 SI 单位制；财务与商业指标按中文语境使用元、件、天等常用单位。具体排放因子应引用最新版 IPCC、GHG Protocol 或企业认可的 LCA 数据库。
...
```mermaid
xychart-beta
    title "微笑曲线：人形机器人价值链附加值示意"
    x-axis ["上游材料", "零部件", "子系统", "整机装配", "品牌/软件/服务"]
    y-axis "附加值" 0 --> 100
    line [80, 60, 50, 40, 90]
```
