---
$id: rel_ent_paper_deniz_identification_of_a_physics_ba_2026_integrates_ent_robot_system_unitree_g1
$schema: ../schema/v1/relationship_schema.json
$version: 1
type: integrates
source:
  id: ent_paper_deniz_identification_of_a_physics_ba_2026
  name:
    en: Identification of a Physics-Based Electrical Power Consumption Model for the
      Unitree G1 Humanoid Arm
    zh: Unitree G1人形机器人手臂基于物理的电力消耗模型辨识
    ko: Unitree G1 휴머노이드 팔의 물리 기반 전력 소비 모델 식별
target:
  id: ent_robot_system_unitree_g1
  name:
    en: Unitree G1 humanoid robot
domains:
  source_domain: 07_ai_models_algorithms
  target_domain: 02_components
description:
  en: The paper identifies and validates the power consumption model using the physical
    Unitree G1 robot.
  zh: 本文使用物理Unitree G1机器人识别并验证功耗模型。
  ko: 본 논문은 실제 Unitree G1 로봇을 사용하여 전력 소비 모델을 식별하고 검증합니다.
verification:
  status: partially_verified
  reviewed_by: human_and_ai
  reviewed_at: '2026-06-22'
  confidence: medium
  notes: 'Proposed by AI extraction. Source citation: Model parameters are identified
    from experimental data collected on a physical Unitree G1 using onboard power
    measurements as the regression target.'
sources:
- id: src_001
  type: paper
  title: Identification of a Physics-Based Electrical Power Consumption Model for
    the Unitree G1 Humanoid Arm
  url: https://arxiv.org/abs/2606.15915
  date: '2026'
  accessed_at: '2026-06-22'
---



