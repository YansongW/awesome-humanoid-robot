---
$id: rel_ent_paper_x_ionet_cross_platform_inertial_odometry_2025_uses_ent_component_imu_2024
...
verification:
  ...
  confidence: medium
  notes: 'Mined by build_latent_relationships.py rule p3_llm_typed. Evidence: 证据明确说明X-IONet仅使用单个IMU进行操作，即X-IONet使用Inertial
    Measurement Unit。 | 证据: To tackle this challenge, we introduce X-IONet, a cross-platform inertial odometry framework that
    operates solely using a single Inertial Measurement Unit (IMU).'
sources:
- id: src_001
  type: other
