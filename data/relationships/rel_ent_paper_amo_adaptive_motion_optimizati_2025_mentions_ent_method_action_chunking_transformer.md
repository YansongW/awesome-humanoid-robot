---
$id: rel_ent_paper_amo_adaptive_motion_optimizati_2025_mentions_ent_method_action_chunking_transformer
...
description:
  en: 'AMO: Adaptive Motion Optimization for Hyper-Dexterous Humanoid Whole-Body Control mentions Action Chunking with Transformers
    (ACT).'
  zh: AMO｜超灵巧人形全身控制的自适应运动优化提及动作分块变压器（ACT）。
verification:
  status: unverified
  reviewed_by: ai
  reviewed_at: '2026-08-06'
  confidence: low
  notes: 'Mined by build_latent_relationships.py rule p5b_acronym. Evidence: 这种设计牺牲了部分全身协调性（作者也承认），但换来了模块化：上半身可以用优化方法（IK）或模仿学习（ACT）独立控制，下半身策略只需跟踪躯干姿态和高度命令。'
sources:
- id: src_001
  type: other
