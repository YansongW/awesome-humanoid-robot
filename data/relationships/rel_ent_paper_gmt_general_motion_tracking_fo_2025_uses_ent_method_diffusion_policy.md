---
$id: rel_ent_paper_gmt_general_motion_tracking_fo_2025_uses_ent_method_diffusion_policy
...
verification:
  ...
  confidence: medium
  notes: 'Mined by build_latent_relationships.py rule p3_llm_typed. Evidence: 论文明确说明生成模型层采用扩散策略（Diffusion Policy）技术。 | 证据:
    随后，生成模型层采用扩散策略（Diffusion Policy）或流匹配（Flow Matching）技术，将动作生成建模为从噪声到可执行轨迹的逆向扩散过程，能够在多模态动作分布中高效采样。'
sources:
- id: src_001
  type: other
