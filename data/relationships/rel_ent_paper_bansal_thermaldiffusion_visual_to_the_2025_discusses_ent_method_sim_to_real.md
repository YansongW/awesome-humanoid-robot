---
$id: rel_ent_paper_bansal_thermaldiffusion_visual_to_the_2025_discusses_ent_method_sim_to_real
$schema: ../schema/v1/relationship_schema.json
$version: 1
type: discusses
source:
  id: ent_paper_bansal_thermaldiffusion_visual_to_the_2025
  name:
    en: 'ThermalDiffusion: Visual-to-Thermal Image-to-Image Translation for Autonomous Navigation'
    zh: ThermalDiffusion：用于自主导航的视觉到热成像图像转换
target:
  id: ent_method_sim_to_real
  name:
    en: Sim-to-Real Transfer
    zh: Sim-to-Real迁移
domains:
  source_domain: 07_ai_models_algorithms
  target_domain: 07_ai_models_algorithms
description:
  en: 'ThermalDiffusion: Visual-to-Thermal Image-to-Image Translation for Autonomous Navigation discusses Sim-to-Real Transfer.'
  zh: ThermalDiffusion：用于自主导航的视觉到热成像图像转换discussesSim-to-Real迁移。
verification:
  status: partially_verified
  reviewed_by: ai
  reviewed_at: '2026-07-16'
  confidence: medium
  notes: 'Mined by build_latent_relationships.py rule p6_llm_link. Evidence: 论文旨在增强自主导航数据集，涉及仿真到现实的迁移'
sources:
- id: src_001
  type: other
  title: KG body of ent_paper_bansal_thermaldiffusion_visual_to_the_2025
  url: https://kg.rounds-tech.com/entry/ent_paper_bansal_thermaldiffusion_visual_to_the_2025/
  accessed_at: '2026-07-16'
---
