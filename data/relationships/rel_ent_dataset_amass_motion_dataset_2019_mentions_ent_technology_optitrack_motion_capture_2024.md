---
$id: rel_ent_dataset_amass_motion_dataset_2019_mentions_ent_technology_optitrack_motion_capture_2024
$schema: ../schema/v1/relationship_schema.json
$version: 1
type: mentions
source:
  id: ent_dataset_amass_motion_dataset_2019
  name:
    en: AMASS Motion Dataset
    zh: AMASS 动作数据集
target:
  id: ent_technology_optitrack_motion_capture_2024
  name:
    en: OptiTrack Motion Capture
    zh: OptiTrack 动作捕捉
domains:
  source_domain: 09_data_datasets
  target_domain: 09_data_datasets
description:
  en: AMASS Motion Dataset mentions OptiTrack Motion Capture.
  zh: AMASS 动作数据集提及OptiTrack 动作捕捉。
verification:
  status: unverified
  reviewed_by: ai
  reviewed_at: '2026-07-16'
  confidence: low
  notes: 'Mined by build_latent_relationships.py rule p4_wiki_ch21. Evidence: 在 Wiki 第 21 章同一小节共现'
sources:
- id: src_001
  type: other
  title: Wiki 第 21 章
  url: https://kg.rounds-tech.com/wiki/chapters/chapter-21/
  accessed_at: '2026-07-16'
---
