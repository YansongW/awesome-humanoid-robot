---
$id: rel_ent_process_p1_mentions_ent_process_p16_3_3
$schema: ../schema/v1/relationship_schema.json
$version: 1
type: mentions
source:
  id: ent_process_p1
  name:
    en: Requirements Definition and System Design (Concept / Pre-A)
    zh: 需求定义与系统方案（Concept / Pre-A）
target:
  id: ent_process_p16_3_3
  name:
    en: PPAP / Mass Production Readiness Assessment
    zh: PPAP / 量产 readiness 评估
domains:
  source_domain: 06_design_engineering
  target_domain: 05_mass_production
description:
  en: Requirements Definition and System Design (Concept / Pre-A) mentions PPAP / Mass Production Readiness Assessment.
  zh: 需求定义与系统方案（Concept / Pre-A）提及PPAP / 量产 readiness 评估。
verification:
  status: unverified
  reviewed_by: ai
  reviewed_at: '2026-07-16'
  confidence: low
  notes: 'Mined by build_latent_relationships.py rule p4_wiki_ch13. Evidence: 在 Wiki 第 13 章同一小节共现'
sources:
- id: src_001
  type: other
  title: Wiki 第 13 章
  url: https://kg.rounds-tech.com/wiki/chapters/chapter-13/
  accessed_at: '2026-07-16'
---
