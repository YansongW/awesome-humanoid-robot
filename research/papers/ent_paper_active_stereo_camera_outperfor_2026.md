...
summary:
  ...
  ko: 인간형 로봇 조작의 ACT 모방 학습에서, 능동 스테레오 카메라는 다중 센서 설정보다 우수한 성능을 보인다. 카메라 이미지/다중 시점 관측, 본체 상태와 관절 시퀀스, 접촉 힘/촉각
    신호를 추적 가능한 신체 목표로 변환하고, ACT/행동 복제 모방 학습, MM-DiT/Transformer 액션 헤드 훈련 또는 결합 전신 정책을 통해 최종적으로 액션 chunk/token을
    출력한다. 핵심은 시연 궤적을 지도 학습 가능한 액션 예측 문제로 압축하고, 액션 chunk 또는 폐루프 실행을 통해 시간적 지터를 줄이는 것이다.
domains:
- 02_components
...
verification:
  ...
  confidence: low
  notes: 'Imported from WeChat curated list (143). Institution: Imitation Learning for Humanoid Manipulation.
    Full title: Active Stereo-Camera Outperforms Multi-Sensor Setup in ACT Imitation Learning for Humanoid
    Manipulation. English name/summary machine-translated from Chinese by scripts/backfill_en_translations.py.
    [2026-08-05] body rewritten as full-text six-section deep read (.staging/deep_read batch2, DeepSeek
    deepseek-chat T<=0.3, arXiv HTML full text); en/ko sections regenerated by translate pipeline. 深读+数字白名单复核通过
    2026-08-10（批量二）；等级 ai_fulltext_verified（AI 全文核验），schema v1 status 枚举不含该值，按数据纪律记为 verified。 | WP3 marker
    fix 2026-08-10: summary.ko translated zh->ko (DeepSeek).'
sources:
- id: src_001
  type: website
