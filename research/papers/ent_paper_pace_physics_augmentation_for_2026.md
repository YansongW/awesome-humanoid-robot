...
summary:
  ...
  ko: 이 작업은 먼저 카메라 이미지/다중 시점 관측, 본체 상태와 관절 시퀀스, 인간 비디오/모션 캡처 궤적으로부터 장면, 목표 또는 운동 표현을 복원한 다음, PPO/RL 정책 훈련을
    통해 관절 위치/토크 명령, 지형/장면 표현을 생성한다. 핵심은 PPO/RL 정책 훈련을 동일한 훈련/배포 파이프라인에 통합하여 고수준 목표와 저수준 동작 사이의 단절을 줄이는 것이다.
domains:
