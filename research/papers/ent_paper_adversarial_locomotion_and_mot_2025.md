...
summary:
  ...
  ko: '이 연구는 주로 데이터 폐루프(data closed-loop) 문제를 해결한다: 온톨로지 상태와 관절 시퀀스, 원격 조작/외골격 데이터, 시뮬레이션 상호작용 데이터를 통해 인간의
    조작과 로봇 상태를 수집하고, 이를 확산 정책(diffusion policy)/흐름 매칭(flow matching), 전신 제어기(WBC)/MPC를 통해 훈련 가능하고 재사용 가능한
    관절 위치/토크 명령, 전신 궤적/동작 시퀀스, 저수준 제어기 목표로 변환한다. 핵심은 동작 생성을 조건부 생성 문제로 간주하고, 확산 또는 흐름 매칭을 사용하여 다중 모드 동작
