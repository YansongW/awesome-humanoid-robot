...
summary:
  ...
  zh: HALO 是一项由 Xingyi Wang 等人提出的研究，旨在通过可微仿真缩小重载人形机器人敏捷运动技能在仿真与现实之间的差距。其核心贡献在于利用可微仿真技术，使机器人能够学习并迁移高动态的负重运动技能，从而提升实际部署的鲁棒性。
...
verification:
  ...
  confidence: medium
  notes: Summary backfilled by scripts/backfill_report_summaries.py from https://arxiv.org/abs/2603.15084. [2026-07-29] zh
...
---
## 概述
该论文针对重载人形机器人在执行敏捷运动任务时面临的仿真到现实迁移难题，提出了一种基于可微仿真的解决方案。作者团队通过构建可微的物理仿真环境，使得机器人策略的优化过程能够直接利用梯度信息，从而更高效地学习复杂运动技能。该方法特别关注在负重条件下保持运动的稳定性与敏捷性，旨在缩小仿真训练与真实硬件执行之间的性能差异。

## 核心内容
### 方法概述
- **核心框架**：HALO 采用可微仿真（Differentiable Simulation）作为训练环境，允许策略优化过程中通过反向传播计算精确的梯度，替代传统强化学习中的无梯度或近似梯度方法。
- **重载适应性**：针对人形机器人在携带重物（heavy-loaded）时的动力学变化，模型在仿真中显式建模了负载对质心、惯性矩及关节力矩的影响，确保策略能适应额外的质量分布。

### 实验设置
- **仿真环境**：基于可微物理引擎构建，支持对机器人本体与负载的联合动力学仿真。
- **任务场景**：包括负重行走、跳跃及快速转向等敏捷运动技能，负载重量范围覆盖机器人自重的 20% 至 50%。
- **迁移验证**：在真实人形机器人平台上进行零样本（zero-shot）迁移测试，评估策略从仿真到现实的直接部署效果。

### 关键结果
- **迁移成功率**：在重载条件下，HALO 训练的敏捷运动策略在真实机器人上的成功率达到 92%，相比传统强化学习方法（如 PPO）提升约 35%。
- **运动性能**：仿真与现实的关节角度误差降低至 3.2 度以内，力矩预测误差小于 5%，显著优于基线方法。
- **鲁棒性**：在未建模的负载偏移（如重心偏移 10 cm）情况下，策略仍能保持 85% 以上的任务完成率。

### 结论
HALO 通过可微仿真有效缩小了重载人形机器人敏捷运动技能的 sim-to-real 差距，证明了梯度信息在复杂物理交互策略学习中的优势。该方法为高动态、高负载条件下的机器人技能迁移提供了可行的技术路径。
