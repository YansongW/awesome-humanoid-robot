...
summary:
  ...
  ko: '이 연구는 주로 데이터 폐루프(data closed-loop) 문제를 해결한다: 카메라 이미지/다시점 관측, 원격 조작/외골격 데이터 수집을 통해 인간의 조작과 로봇 상태를
    수집하고, 이후 ACT/행동 클로닝 모방 학습, 확산 정책/흐름 매칭을 통해 훈련 가능하고 재사용 가능한 실행 가능한 동작 명령으로 변환한다. 핵심은 동작 생성을 조건부 생성 문제로
    간주하고, 확산 또는 흐름 매칭을 사용하여 다중 모드 동작 분포에서 실행 가능한 궤적을 샘플링하는 것이다.'
domains:
