...
summary:
  en: A 2026 survey that defines World Action Models (WAMs) as embodied foundation
    models unifying predictive world modeling with action generation, and organizes
    existing methods into Cascaded and Joint architectures.
  zh: 2026 年综述，将 World Action Models（WAMs）定义为统一预测性世界建模与动作生成的具身基础模型，并将现有方法组织为级联式（Cascaded）与联合式（Joint）架构。
  ko: 2026년 서베이로, World Action Models(WAMs)를 예측적 월드 모델링과 액션 생성을 통합하는 엠바디드 파욍데이션 모델로
    정의하고, 기존 방법을 Cascaded 및 Joint 아키텍처로 분류함.
domains:
...
verification:
  ...
  confidence: high
  notes: Paper retrieved from arXiv; GitHub companion repository provides leaderboard
    and taxonomy. Claims about scope based on abstract and repository overview.
sources:
- id: src_wam_paper
  type: paper
...
- id: ent_paper_wang_vla_survey_2026
  ...
  description:
    en: The WAM survey extends the data-centric VLA survey by focusing on models that
      jointly predict future states and actions.
...
- id: ent_dataset_omniaction
  relationship: cites
  description:
    en: The WAM survey discusses omni-modal and internet-scale data ecosystems fueling
      WAM development.
...
- id: ent_benchmark_libero_plus
  relationship: cites
  description:
    en: The WAM survey cites robustness benchmarks such as LIBERO-plus as part of
      the emerging WAM evaluation landscape.
