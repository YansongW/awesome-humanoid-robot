...
summary:
  ...
  ko: 'Retargeting은 주로 데이터 폐루프를 해결한다: 인간 비디오/모션 캡처 궤적, 원격 조작/외골격 데이터, 접촉력/촉각 신호를 통해 인간의 조작과 로봇 상태를 수집하고,
    이를 PPO/RL 정책 훈련, IK/동작 리타겟팅, 계층적 스킬/전문가 정책을 통해 훈련 가능하고 재사용 가능한 전신 궤적/동작 시퀀스, 저수준 컨트롤러 목표로 변환한다. 핵심은
    작업을 라우팅 가능한 스킬 또는 전문가 정책으로 분해하고, 상위 모듈이 실행 중에 이를 선택하고 결합하는 것이다.'
domains:
