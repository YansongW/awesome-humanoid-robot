...
summary:
  ...
  ko: 'LIFT: Towards Bridging the Gap between Large-Scale Pretraining and Efficient Finetuning for Humanoid
    Control은 휴머노이드 로보틱스의 Sim-to-Real에 관한 논문이다. LIFT는 대규모 SAC 사전 훈련과 물리적 사전 지식 기반 세계 모델 미세 조정을 결합하여, 휴머노이드
    sim-to-real을 1시간으로 단축한다.'
domains:
