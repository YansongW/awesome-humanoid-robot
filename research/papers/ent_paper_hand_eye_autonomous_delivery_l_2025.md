...
summary:
  ...
  ko: 이 작업은 먼저 카메라 이미지/다중 시점 관측, 본체 상태와 관절 시퀀스, 인간 비디오/모션 캡처 궤적으로부터 장면, 목표 또는 운동 표현을 복원하고, 이종 모션 캡처와 합성
    균형 데이터, 전신 제어기/WBC/MPC, 계층적 스킬/전문가 정책을 사용하여 저수준 제어기 목표, 내비게이션/목표 도달을 생성한다. 핵심은 작업을 라우팅 가능한 스킬 또는 전문가
    정책으로 분해하고, 상위 모듈이 실행 중에 이를 선택하고 결합하는 것이다.
domains:
...
verification:
  ...
  notes: 'Structured body generated by scripts/backfill_critical_paper_bodies.py via DeepSeek API from entry
    metadata. | WP1 dedup merge 2026-08-06: merged ent_paper_hand_eye_autonomous_delivery_l_2025, ent_paper_hand_eye_autonomous_delivery_l_2025
