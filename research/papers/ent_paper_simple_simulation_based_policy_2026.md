...
summary:
  ...
  ko: SIMPLE의 구현 경로는 먼저 카메라 이미지/다중 시점 관측, 본체 상태와 관절 시퀀스, 원격 조작/외골격 데이터를 다중 모달 표현으로 인코딩한 다음, VLA 다중 모달 동작
    모델, 세계 모델/비디오 예측을 통해 전신 궤적/동작 시퀀스를 예측하는 것이다. 핵심은 비디오/세계 모델이 예측 가능한 물리적 사전 지식을 제공하도록 하고, 이후 동작 헤드가 의미론적
    목표를 연속 제어로 변환하는 것이다.
domains:
