...
summary:
  ...
  zh: FRoM-W1 把语言指令、本体状态与关节序列、仿真交互数据转成可跟踪的身体目标，并通过PPO/RL 策略训练、ACT/行为克隆模仿学习、VLM 语义规划/路由训练或组合全身策略，最终输出全身轨迹/动作序列、低层控制器目标。关键点是把示范轨迹压成可监督的动作预测问题，再通过动作
    chunk 或闭环执行降低时序抖动。
  ko: FRoM-W1은 언어 명령, 본체 상태와 관절 시퀀스, 시뮬레이션 상호작용 데이터를 추적 가능한 신체 목표로 변환하고, PPO/RL 정책 훈련, ACT/행동 복제 모방 학습,
    VLM 의미론적 계획/라우팅 훈련 또는 조합된 전신 정책을 통해 최종적으로 전신 궤적/동작 시퀀스와 저수준 제어기 목표를 출력한다. 핵심은 시연 궤적을 감독 가능한 동작 예측 문제로
    압축한 다음, 동작 청크 또는 폐루프 실행을 통해 시간적 지터를 줄이는 것이다.
domains:
...
verification:
  ...
  notes: 'Structured body generated by scripts/backfill_critical_paper_bodies.py via DeepSeek API from entry
    metadata. | WP1 dedup merge 2026-08-06: merged ent_paper_from_w1_towards_general_humano_2026, ent_paper_from_w1_towards_general_humano_2026
