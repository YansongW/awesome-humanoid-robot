...
summary:
  ...
  ko: 휴머노이드는 주로 데이터 폐루프(data closed-loop)를 해결한다. 언어 명령, 카메라 이미지/다중 시점 관측, 원격 조작/외골격 데이터 수집을 통해 인간의 조작과 로봇
    상태를 획득하고, 이를 확산 정책(diffusion policy)/흐름 매칭(flow matching)을 통해 훈련 가능하고 재사용 가능한 실행 가능한 행동 명령으로 변환한다. 핵심은
    행동 생성을 조건부 생성 문제로 간주하고, 확산 또는 흐름 매칭을 사용하여 다중 모달 행동 분포에서 실행 가능한 궤적을 샘플링하는 것이다.
domains:
