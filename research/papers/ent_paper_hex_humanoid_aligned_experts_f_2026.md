...
summary:
  ...
  ko: HEX의 구현 경로는 먼저 본체 상태와 관절 시퀀스를 다중 모달 표현으로 인코딩한 다음, PPO/RL 정책 훈련, VLA 다중 모달 동작 모델, VLM 의미 계획/라우팅 예측
    전신 궤적/동작 시퀀스, 저수준 제어기 목표를 사용하는 것이다. 핵심은 VLM의 의미 이해를 유지하면서 로봇 상태와 동작 헤드를 추가하여 언어 계획에만 머무르지 않도록 하는 것이다.
domains:
