...
summary:
  ...
  zh: AgiBot World Colosseo 是一个由智元机器人（AgiBot）构建的大规模人形机器人操作数据集，包含超过100万条轨迹，覆盖217个任务。基于该数据集训练的通用策略模型 Genie Operator-1 (GO-1)
    利用潜在动作表征，在真实世界任务中比 Open X-Embodiment 提升30%，比 RDT 提升32%。
...
verification:
  ...
  confidence: medium
  notes: Abstract backfilled by scripts/backfill_paper_abstracts.py from http://arxiv.org/abs/2503.06669v4. [2026-07-29] zh
...
---
## 概述
AgiBot World 平台通过100多台 AgiBot G1 机器人采集数据，实现了比现有数据集高一个数量级的数据规模。该平台采用标准化采集流程并加入人工验证，确保数据质量与多样性，且支持从夹爪到灵巧手及视觉触觉传感器的扩展。基于此数据训练的 GO-1 策略通过潜在动作表征最大化数据利用率，展现出随数据量增长的可预测性能提升。

## 核心内容
### 核心贡献
- **AgiBot World 数据集**：包含超过100万条轨迹，覆盖5个部署场景中的217个任务，数据规模较现有数据集提升一个数量级。
- **标准化采集流程**：采用人工在环验证，保证数据质量与分布多样性，支持从夹爪到灵巧手及视觉触觉传感器的扩展。
- **Genie Operator-1 (GO-1)**：一种利用潜在动作表征的通用策略，最大化数据利用率，性能随数据量增长呈可预测提升。

### 实验设置与结果
- **性能对比**：基于 AgiBot World 预训练的策略在域内与域外场景中，平均性能比基于 Open X-Embodiment 训练的模型提升30%。
- **复杂任务表现**：GO-1 在真实世界的灵巧操作与长时域任务中成功率超过60%，比先前 RDT 方法提升32%。
- **开源计划**：数据集、工具与模型全部开源，旨在推动大规模高质量机器人数据的民主化获取。
