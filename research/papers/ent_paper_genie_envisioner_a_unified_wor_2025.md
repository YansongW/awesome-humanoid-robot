...
summary:
  ...
  ko: Genie Envisioner의 구현 경로는 먼저 언어 명령, 카메라 이미지/다중 시점 관측, 본체 상태와 관절 시퀀스를 다중 모달 표현으로 인코딩한 뒤, ACT/행동 클로닝
    모방 학습, 확산 정책/흐름 매칭, 세계 모델/비디오 예측을 통해 지형/장면 표현을 예측한다. 핵심은 동작 생성을 조건부 생성 문제로 간주하고, 확산 또는 흐름 매칭을 사용하여 다중
    모달 동작 분포에서 실행 가능한 궤적을 샘플링하는 것이다.
domains:
