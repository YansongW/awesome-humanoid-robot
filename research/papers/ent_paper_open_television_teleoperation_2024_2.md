...
summary:
  ...
  ko: '이 연구는 주로 데이터 폐루프(data closed-loop) 문제를 해결한다: 카메라 이미지/다중 시점 관측, 본체 상태와 관절 시퀀스, 원격 조작/외골격 데이터를 통해 인간의
    조작과 로봇 상태를 수집하고, 이후 ACT/행동 클로닝 모방 학습, 확산 정책/플로우 매칭, MM-DiT/Transformer 액션 헤드를 통해 훈련 가능하고 재사용 가능한 관절
    위치/토크 명령, 전신 궤적/행동 시퀀스, 엔드 이펙터/손목-손 목표로 변환한다. 핵심은 동작 생성을 조건부 생성 문제로 간주하고, 확산 또는 플로우 매칭을 사용하여 다중 모달 동작
