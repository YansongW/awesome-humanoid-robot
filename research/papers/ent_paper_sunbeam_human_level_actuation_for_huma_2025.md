...
---
## 概述
当前人形机器人常宣称达到“人类级”驱动，但缺乏量化标准。峰值扭矩或速度指标无法反映关节在任务相关姿态和速率下同时提供扭矩、功率和耐力的能力。本文提出的框架包含三个核心组件：基于ISB标准的运动自由度图谱统一了人机关节坐标系；人类等效包络线（HEE）通过同时测量扭矩和功率在任务频段（行走、爬楼梯、举重、伸手和手部动作）的匹配度定义关节需求；人类级驱动评分（HLAS）则聚合工作空间覆盖率、HEE覆盖率、扭矩模式带宽、效率和热可持续性六个物理因素。该框架既可作为人形机器人设计规范，也可作为驱动系统比较的基准标准。

## 核心内容
### 方法架构
- **运动自由度图谱（DoF Atlas）**：采用ISB标准定义关节坐标系与运动范围，确保人机关节在相同参考系下比较
- **人类等效包络线（HEE）**：在特定关节角度和速率(q,ω)下，同时验证机器人是否满足人类扭矩与功率要求，并按任务频段（行走、爬楼梯、举重、伸手、手部动作）的正机械功加权
- **人类级驱动评分（HLAS）**：聚合六个物理因素：
  - 工作空间覆盖率（ROM与DoF）
  - HEE覆盖率
  - 扭矩模式带宽
  - 效率
...
### 实验设置
- 测量协议：使用测力计、电功率监测和热测试获取HLAS所有输入参数
- 示例验证：通过多关节人形机器人实例展示HLAS计算过程，揭示峰值扭矩规格无法体现的驱动权衡（如传动比与带宽、效率的冲突）

### 关键结论
- 框架基于已发表的人类生物力学数据构建，所有组件均可通过可复现实验获得
- HLAS能暴露传统峰值扭矩指标掩盖的设计权衡，例如高传动比提升扭矩但降低带宽和效率
- 该框架同时服务于人形机器人开发的设计规范和驱动系统比较的基准标准
