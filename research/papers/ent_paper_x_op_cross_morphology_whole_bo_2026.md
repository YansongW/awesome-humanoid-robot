...
summary:
  ...
  ko: 'X-OP: Cross-Morphology Whole-Body Teleoperation via MPC Retargeting은 휴머노이드 로보틱스 분야의 원격 조작(teleoperation)에
    관한 논문입니다. 단일 Apple Vision Pro로 구동되며, 형태 간 재훈련 없이 작동하는 계층적 전신 원격 조작 방식을 제안합니다. MPC 리타게팅(MPC retargeting)이
    조작자의 의도와 동역학적 실현 가능성을 동시에 고려합니다.'
domains:
