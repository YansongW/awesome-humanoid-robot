...
summary:
  ...
  ko: Any2Any는 시각, 상태 및 동작 데이터를 추적 가능한 신체 목표로 변환하고, 소스/타깃 로봇의 운동학 정렬, PEFT 동역학 적응, 전신 제어기/WBC/MPC 훈련 또는
    결합 전신 정책을 통해 최종적으로 전신 궤적/동작 시퀀스 및 저수준 제어기 목표를 출력합니다. 핵심은 먼저 소스/타깃 로봇의 상태 및 동작 공간을 정렬한 후, 동역학에 민감한 모듈만
    미세 조정하여 원래 정책의 운동 사전 지식을 최대한 보존하는 것입니다.
domains:
...
verification:
  ...
  notes: 'Structured body generated by scripts/backfill_critical_paper_bodies.py via DeepSeek API from entry
    metadata. | WP1 dedup merge 2026-08-06: merged ent_paper_any2any_efficient_cross_embodi_2026 into this
