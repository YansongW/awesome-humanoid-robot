...
summary:
  ...
  ko: '이 연구는 주로 데이터 폐루프(data closed-loop) 문제를 해결한다: 카메라 이미지/다시점 관측, 로봇 상태와 관절 시퀀스, 원격 조작/외골격 데이터 수집을 통해
    인간의 조작과 로봇 상태를 획득하고, 이후 교사-학생 지식 전이(teacher-student knowledge transfer), 전신 제어기(WBC)/MPC, 폐루프 오류 수정/인간
    개입을 통해 훈련 가능하고 재사용 가능한 실행 가능한 동작 명령으로 변환한다. 핵심은 특권 정보(privileged information)를 사용하여 교사 정책을 훈련하고, 그 능력을
    배포 시 관측만 사용할 수 있는 학생 정책으로 증류(distillation)하는 것이다.'
domains:
...
verification:
  ...
  notes: 'Structured body generated by scripts/backfill_critical_paper_bodies.py via DeepSeek API from entry
    metadata. | WP1 dedup merge 2026-08-06: merged ent_paper_opening_the_sim_to_real_door_f_2025_1 into
