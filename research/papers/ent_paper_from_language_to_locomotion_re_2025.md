...
summary:
  ...
  ko: 이 작업은 먼저 언어 명령, 카메라 이미지/다중 시점 관측, 인간 비디오/모션 캡처 궤적으로부터 장면, 목표 또는 운동 표현을 복원한 다음, 확산 정책/플로우 매칭, MM-DiT/Transformer
    액션 헤드, IK/동작 리타게팅을 통해 전신 궤적/동작 시퀀스를 생성한다. 핵심 포인트는 동작 생성을 조건부 생성 문제로 간주하고, 확산 또는 플로우 매칭을 사용하여 다중 모달 동작
    분포에서 실행 가능한 궤적을 샘플링하는 것이다.
domains:
