...
summary:
  ...
  ko: PhysHSI는 먼저 카메라 이미지/다중 시점 관측, 본체 상태와 관절 시퀀스, 깊이/포인트 클라우드/높이 맵에서 장면, 목표 또는 운동 표현을 복원하고, 이후 AMP/운동 사전,
    DINO/시각 특징 추출을 사용해 지형/장면 표현을 생성한다. 핵심은 AMP/운동 사전과 DINO/시각 특징 추출을 동일한 훈련/배포 파이프라인에 배치하여 고수준 목표와 저수준 동작
    사이의 단절을 줄이는 것이다.
domains:
