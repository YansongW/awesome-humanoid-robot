...
summary:
  ...
  ko: '이 연구는 주로 데이터 폐루프(data closed-loop) 문제를 해결한다: 카메라 이미지/다시점 관측, 로봇 상태와 관절 시퀀스, 인간 영상/모션 캡처 궤적을 통해 인간
    조작과 로봇 상태를 수집하고, 이를 IK/동작 리타게팅을 통해 훈련 가능하고 재사용 가능한 전신 궤적/동작 시퀀스 및 저수준 제어기 목표로 변환한다. 핵심은 IK/동작 리타게팅을
    동일한 훈련 및 배포 파이프라인에 통합하여 고수준 목표와 저수준 동작 사이의 단절을 줄이는 것이다.'
domains:
