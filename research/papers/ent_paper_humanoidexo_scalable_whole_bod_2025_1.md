...
summary:
  ...
  ko: HumanoidExo는 주로 데이터 폐루프(Data Closed-Loop) 문제를 해결한다. 인간 비디오/모션 캡처 궤적, 원격 조작/외골격 데이터 수집을 통해 인간의 조작과
    로봇 상태를 획득하고, 이후 확산 정책(Diffusion Policy)/흐름 매칭(Flow Matching), 전신 제어기(WBC)/MPC, 계층적 스킬/전문가 정책을 통해 훈련
    가능하고 재사용 가능한 전신 궤적/동작 시퀀스로 변환한다. 핵심은 동작 생성을 조건부 생성 문제로 간주하고, 확산 또는 흐름 매칭을 사용하여 다중 모달 동작 분포에서 실행 가능한
    궤적을 샘플링하는 것이다.
domains:
- 09_data_datasets
...
verification:
  ...
  notes: 'Structured body generated by scripts/backfill_critical_paper_bodies.py via DeepSeek API from entry
    metadata. | WP1 dedup merge 2026-08-06: merged ent_paper_humanoidexo_scalable_whole_bod_2025_1 into
