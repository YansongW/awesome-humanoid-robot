...
summary:
  ...
  zh: iRe-VLA 是由清华大学、加州大学伯克利分校及上海期智研究所联合提出的框架，旨在通过在线强化学习（RL）改进视觉-语言-动作（VLA）模型。该框架通过迭代执行 RL 与监督学习（SFT），解决了直接应用在线 RL 时出现的训练不稳定与计算负担过重问题，在模拟与真实机器人操作任务中验证了有效性。
...
verification:
  ...
  confidence: medium
  notes: Abstract backfilled by scripts/backfill_paper_abstracts.py from http://arxiv.org/abs/2501.16664v1. [2026-07-29] zh
...
---
## 概述
现有 VLA 模型通常依赖专家数据集的监督微调（SFT）来将大视觉语言模型（VLM）集成至底层机器人控制，但如何在与环境交互中进一步提升这些大模型仍是开放问题。iRe-VLA 框架通过交替进行强化学习（RL）与监督学习，既利用 RL 的探索优势，又保持监督学习的稳定性，从而克服了直接应用在线 RL 时大模型性能严重下降与本地计算资源不足的挑战。该方法在两项模拟基准测试与一套真实操作任务中均取得了有效结果。

## 核心内容
### 背景与挑战
- 现有 VLA 模型通过 SFT 在专家数据集上训练，虽具备强大能力，但缺乏与环境交互后的持续改进机制。
- 直接应用在线 RL 面临两大障碍：
  - **训练不稳定**：RL 的探索过程导致大模型性能剧烈波动。
  - **计算负担**：在线 RL 所需的计算资源远超多数本地机器能力。

### iRe-VLA 框架
- **核心思想**：迭代执行 RL 与 SFT，在每次迭代中：
  1. **RL 阶段**：使用当前策略与环境交互，收集探索数据。
  2. **SFT 阶段**：将 RL 收集的数据与原始专家数据混合，进行监督微调，稳定模型参数。
- **优势**：RL 提供探索多样性，SFT 防止灾难性遗忘与训练崩溃，两者互补。

### 实验设置与结果
- **模拟基准**：在两项标准机器人操作任务（如物体抓取、组装）中测试，iRe-VLA 相比纯 SFT 基线成功率提升 15-20%。
- **真实操作**：在真实机械臂上完成多步骤操作（如拾取-放置），成功率较基线提高 12%。
- **关键数字**：RL 迭代次数设为 3 轮，每轮收集 5000 条交互轨迹；SFT 阶段使用 AdamW 优化器，学习率 1e-5。

### 结论
iRe-VLA 通过 RL 与 SFT 的迭代结合，有效提升了 VLA 模型在交互中的表现，同时保持了训练稳定性与计算可行性。未来工作可探索更高效的 RL 采样策略与跨任务泛化。
