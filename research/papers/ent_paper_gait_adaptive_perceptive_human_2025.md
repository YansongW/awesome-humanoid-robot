...
summary:
  ...
  ko: 이 작업은 먼저 카메라 이미지/다중 시점 관측, 본체 상태와 관절 시퀀스, 깊이/포인트 클라우드/높이 맵으로부터 장면, 목표 또는 운동 표현을 복원한 뒤, 하향식 깊이 카메라와
    U-Net 높이 맵 재구성, 보행 위상/주파수 조절, 교사-학생 지식 전이를 통해 관절 위치/토크 명령을 생성하고, 지형/장면 표현을 활용한다. 핵심은 지형 재구성, 보행 위상,
