...
## 개요
Mimic2DM은 비디오에서 직접 추출한 2D 키포인트 궤적을 통해 제어 정책을 학습하여, 기존 방법들이 3D 모션 재구성에 의존하던 일반화 문제를 해결합니다. 이 프레임워크는 먼저 재투영 오차를 최소화하여 물리 시뮬레이션에서 임의의 2D 참조 모션을 따르는 범용 단일 시점 2D 모션 추적 정책을 훈련합니다. 이 정책이 서로 다르거나 약간 다른 시점에서 얻은 다양한 2D 모션으로 훈련되면, 다중 시점 정보를 집계하여 3D 모션 추적 능력을 획득할 수 있습니다. 또한, 연구팀은 Transformer 기반의 자기회귀 2D 모션 생성기를 개발하여 계층적 제어 프레임워크에 통합함으로써, 추적 정책을 안내할 고품질 2D 참조 궤적을 생성합니다.

## 핵심 내용
### 방법 개요
Mimic2DM의 핵심 혁신은 3D 모션 재구성을 완전히 우회하고 2D 키포인트 궤적을 직접 활용하여 물리 시뮬레이션을 제어하는 것입니다. 프레임워크는 두 가지 주요 구성 요소를 포함합니다:
- **2D 모션 추적 정책**: 재투영 오차를 최소화하여 훈련되며, 정책이 물리 시뮬레이션에서 임의의 2D 참조 모션을 따를 수 있게 합니다. 이 정책은 다양한 시점의 2D 모션 데이터로 훈련된 후, 다중 시점 집계를 통해 3D 모션 추적 능력을 암시적으로 학습합니다.
- **Transformer 자기회귀 생성기**: Transformer 아키텍처를 기반으로 하며, 계층적 제어 프레임워크의 상위 지침 역할을 하는 고품질 2D 참조 궤적을 생성합니다.

### 실험 설정 및 주요 결과
- **훈련 데이터**: 비디오에서 추출한 2D 키포인트 궤적만 사용하며, 3D 모션 데이터는 전혀 필요하지 않습니다.
- **테스트 영역**: 춤, 축구 드리블, 동물 운동 등 다양한 시나리오를 포함하며, 인간-물체 상호작용(HOI) 및 비인간 캐릭터도 포함합니다.
- **주요 성능**:
  - 춤 동작 합성에서 Mimic2DM이 생성한 물리적으로 타당한 동작은 실제 비디오와 높은 일치도를 보였습니다.
  - 축구 드리블 작업에서 명시적 3D 모션 데이터 없이 인간과 공의 상호작용을 성공적으로 시뮬레이션했습니다.
  - 동물 운동(예: 네발 보행)에서 종 간 일반화 능력을 입증했습니다.
- **비교 기준**: 3D 재구성에 의존하는 방법과 비교하여 Mimic2DM은 일반화성과 물리적 타당성에서 크게 향상되었으며, 특히 HOI와 같은 도전적인 시나리오에서 두드러진 성능을 보였습니다.

### 결론
Mimic2DM은 2D 키포인트 궤적에서 직접 3D 캐릭터 모션 제어를 학습할 수 있음을 입증하여, 저비용·고일반화 모션 합성을 위한 새로운 패러다임을 제시합니다. 프로젝트 웹사이트에서 더 많은 데모와 코드를 확인할 수 있습니다: https://jiann-li.github.io/mimic2dm/
