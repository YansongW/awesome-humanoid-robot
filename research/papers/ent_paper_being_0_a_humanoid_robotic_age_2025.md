...
summary:
  ...
  ko: Being-0은 먼저 언어 명령, 카메라 이미지/다중 시점 관측으로부터 장면, 목표 또는 운동 표현을 복원한 다음, VLM 의미론적 계획/라우팅, 계층적 스킬/전문가 정책을 통해
    실행 가능한 동작 명령을 생성한다. 핵심은 작업을 라우팅 가능한 스킬 또는 전문가 정책으로 분해하고, 고위층 모듈이 실행 중에 이를 선택하고 조합하는 것이다.
domains:
