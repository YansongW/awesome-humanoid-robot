...
- id: ent_robomimic
  ...
  description:
    en: Diffusion Policy is evaluated on tasks from the Robomimic proficient-human
      and multi-human demonstration datasets.
    zh: 扩散策略在Robomimic熟练人类与多人类示教数据集的任务上进行了评估。
    ko: 디퓨전 정책은 Robomimic 숙련된 인간 및 다중 인간 시연 데이터 세트의 작업에서 평가되었다.
- id: ent_push_t
  relationship: evaluates_on
  description:
    en: Diffusion Policy is evaluated on the Push-T demonstration dataset and real-world
      Push-T tasks.
    zh: 扩散策略在Push-T示教数据集以及真实世界的Push-T任务上进行了评估。
    ko: 디퓨전 정책은 Push-T 시연 데이터 세트와 실제 Push-T 작업에서 평가되었다.
- id: ent_multimodal_block_pushing
  relationship: evaluates_on
  description:
    en: Diffusion Policy is evaluated on the Multimodal Block Pushing demonstration
      dataset.
    zh: 扩散策略在多模态推块示教数据集上进行了评估。
    ko: 디퓨전 정책은 다중 모드 블록 밀기 시연 데이터 세트에서 평가되었다.
...
- id: ent_ur5_robot_arm
  relationship: uses
  description:
    en: Diffusion Policy uses a UR5 robot arm in real-world experiments.
    zh: 扩散策略在真实世界实验中使用了UR5机械臂。
    ko: 디퓨전 정책은 실제 실험에서 UR5 로봇 팔을 사용하였다.
- id: ent_franka_panda
  relationship: uses
  description:
...
- id: ent_intel_realsense_d415
  relationship: uses
  description:
    en: Diffusion Policy uses Intel RealSense D415 RGB-D cameras for visual observations.
    zh: 扩散策略使用Intel RealSense D415 RGB-D相机获取视觉观测。
    ko: 디퓨전 정책은 시각 관찰을 위해 Intel RealSense D415 RGB-D 카메라를 사용하였다.
