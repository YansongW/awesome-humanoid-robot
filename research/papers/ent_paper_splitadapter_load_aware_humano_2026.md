...
summary:
  ...
  ko: SplitAdapter는 시뮬레이션 상호작용 데이터를 추적 가능한 신체 목표로 변환하고, PPO/RL 정책 훈련, 확산 정책/플로우 매칭, 전신 제어기/WBC/MPC 훈련 또는
    결합 전신 정책을 통해 최종적으로 전신 궤적/동작 시퀀스와 저수준 제어기 목표를 출력한다. 핵심은 동작 생성을 조건부 생성 문제로 간주하고, 확산 또는 플로우 매칭을 사용하여 다중
