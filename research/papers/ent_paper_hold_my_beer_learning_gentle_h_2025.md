...
summary:
  ...
  ko: 먼저 카메라 이미지/다중 시점 관측으로부터 장면, 대상 또는 운동 표현을 복원한 다음, ACT/행동 클로닝 모방 학습을 사용하여 전신 궤적/동작 시퀀스, 말단 실행기/손목-손
    목표, 저수준 제어기 목표를 생성한다. 핵심은 시연 궤적을 지도 학습 가능한 동작 예측 문제로 압축한 후, 동작 청크 또는 폐루프 실행을 통해 시간적 지터를 줄이는 것이다.
domains:
