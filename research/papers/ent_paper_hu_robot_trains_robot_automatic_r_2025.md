...
---
## 概述
RTR 框架的核心创新在于将机械臂作为主动教师，通过提供物理保护、学习进度安排、奖励信号、扰动注入、失败检测和自动重置，解决了真实世界人形机器人强化学习中的安全性和效率问题。该方法还提出一种新的 sim-to-real 迁移流程，通过在真实世界中优化单个动力学编码的隐变量，结合 FiLM 层和域随机化技术，稳定策略迁移。实验在 ToddlerBot 人形机器人上完成两项挑战性任务：行走策略微调以实现精确速度跟踪，以及从零开始学习摆起动作，展示了 RTR 系统在真实世界人形机器人学习中的潜力。

## 核心内容
### 方法架构
- **RTR 框架**：机械臂教师通过柔性接触提供物理支撑，根据学生能力自动调整课程难度（curriculum scheduling），注入扰动（perturbations）增强鲁棒性，并具备失败检测（failure detection）和自动重置（automatic resets）功能，实现长时间连续训练。
- **Sim-to-Real 迁移**：提出动力学感知隐变量优化流程，在仿真中通过域随机化（domain randomization）训练策略，并在真实世界中优化单个动力学编码的隐变量（dynamics-encoded latent variable），结合 FiLM 层（Feature-wise Linear Modulation）调整网络特征，稳定迁移过程。

### 实验设置
- **平台**：ToddBot 人形机器人，配备机械臂教师系统。
- **任务**：
  - **行走速度跟踪微调**：从预训练策略出发，在真实世界中微调以精确跟踪目标速度。
  - **从零开始摆起学习**：无预训练策略，直接学习从地面摆起至站立动作。
- **训练过程**：机械臂教师全程提供物理保护，自动检测失败并重置机器人至初始状态，无需人工干预。

### 关键结果
- **行走速度跟踪**：RTR 框架使机器人能在真实世界中快速收敛到目标速度，速度跟踪误差显著低于纯仿真策略。
- **摆起学习**：从零开始学习成功实现完整摆起动作，训练时间约 2 小时，失败率通过机械臂保护降至接近零。
- **Sim-to-Real 稳定性**：动力学隐变量优化结合 FiLM 层，使策略在真实世界中的行为与仿真高度一致，减少迁移抖动。

### 结论
RTR 框架通过机械臂教师实现安全、高效的自动训练，结合动力学感知隐变量优化，有效解决了真实世界人形机器人强化学习的核心挑战。该方法为未来人形机器人在复杂环境中的自主学习和适应提供了可行方案。
