...
summary:
  ...
  zh: 'Over the next few decades, billions of autonomous, AI-powered robots will work alongside people in factories, perform
    tedious tasks in warehouses, care for the elderly, assist in unsafe disaster areas , deliver packages and food to our
    doorsteps, and eventually help out in our homes. Some will look like us, and many won’t. What is certain is that regardless
    of form factor, robots will all rely heavily on AI in order to deliver real-world value. In 2025, total investments in
    robotics companies reached a record US $40.7 billion, accounting for 9 percent of all venture funding . The multibillion
...
## 개요
로봇 공학에서 단 한 번의 획기적인 AI 돌파구가 해답이 아닐 수도 있다. 한스 피터 브뢴드모(Hans Peter Brøndmo)는 2016년부터 2023년까지 Google X의 부사장으로 재직하며 Everyday Robots를 시작하고 이끌었다.

## 핵심 내용
향후 수십 년 동안 수십억 대의 자율 AI 로봇이 공장에서 사람들과 함께 일하고, 창고에서 지루한 작업을 수행하며, 노인을 돌보고, 위험한 재해 지역에서 지원하며, 우리 집 앞까지 패키지와 음식을 배달하고, 결국에는 가정에서도 도움을 줄 것이다. 일부는 우리와 비슷한 모습을 하고 있을 것이고, 많은 로봇은 그렇지 않을 것이다. 확실한 것은, 폼 팩터와 관계없이 로봇은 모두 현실 세계에서 가치를 창출하기 위해 AI에 크게 의존할 것이라는 점이다. 2025년, 로봇 공학 기업에 대한 총 투자액은 407억 달러로 사상 최고치를 기록했으며, 이는 전체 벤처 투자 자금의 9%를 차지한다. 따라서 수십억 달러 규모의 질문은 이것이다: AI 기반 로봇이 본격적인 경제적 영향을 미치기 위해서는 무엇이 필요할까? 오늘날 많은 로봇 공학 및 AI 기업들은 휴머노이드 로봇이 곧 우리 집에 들어올 것이라는 등 대담한 주장을 하고 있지만, 약속과 현실 사이에는 여전히 큰 격차가 존재한다. 우리와 함께 살고 일하는 로봇에 대한 약속은 오랫동안 공상과학의 영역이었다. 많은 프로그래머들이 이 약속을 현실로 만들기 위해 노력해 왔지만, 물리적 세계는 전통적인 컴퓨터 프로그램이 처리하기에는 너무 복잡하다. AI 덕분에 로봇은 더 이상 프로그래밍되지 않는다. 대신, 로봇은 실제 세계에서 작동하는 법을 학습한다. 충분한 연습을 통해 로봇은 주변 세계를 인식하고 이해하며, 그 세계에 대해 추론하고, 그 추론과 이해를 바탕으로 유용하고 신뢰할 수 있으며 안전한 작업을 수행할 수 있다. 우리 두 사람은 지난 10년 동안 AI와 로봇 공학의 최전선에서 일해 왔다. 한 명은 오리건 주립대학교의 로봇 공학 교수이자 Agility Robotics의 공동 창업자이고, 다른 한 명은 Google X의 Everyday Robots 문샷 프로젝트의 전 CEO이다. 실제 환경에서 AI 기반 로봇을 배치한 경험을 통해 우리는 AI가 단기적으로 복잡한 로봇 시스템에서 큰 이점을 제공할 수 있는 분야와 여전히 공상과학의 경계에 있는 분야에 대한 관점을 얻었다. 우리는 AI가 로봇 공학의 발전에 변곡점을 가져올 것이라고 믿지만, 이는 단일한 ChatGPT 스타일의 돌파구가 아니라 다양한 AI 도구의 조정된 시스템을 잘 엔지니어링하여 적용함으로써 이루어질 것이다. AI에 대한 기대가 무엇이 가능할지에 대한 불확실성과 맞물려 있는 상황에서, 로봇 공학에서 AI를 정의할 다섯 가지 엄중한 진실을 소개한다.

1. **YouTube와 현실의 격차는 실재한다**
   수년 동안 우리는 휴머노이드 로봇이 댄스 플로어부터 장애물 코스까지 모든 곳에서 놀라운 동작을 수행하는 YouTube 동영상을 보아 왔다. 로봇 공학 내부에서는 "YouTube 로봇 동영상을 믿지 마라"는 지식이 있다. 구조화되지 않은 인간 환경에서 실제 작업을 수행할 수 있는 실제 로봇과 신중하게 대본이 작성되고 편집된 로봇 공연 사이의 격차는 여전히 크다. 최근 많은 주목을 받은 공연은 중국 2026년 춘절 갈라에서 Unitree 휴머노이드 로봇이 어린이들과 함께 무술 공연을 선보인 것이었다. 인상적이었지만, 이는 모든 것이 사전에 세심하게 안무되고 계획된, 오랜 전통의 엄격하게 대본화된 로봇 공연의 연장선에 있다. 저수준 제어, 동기화, 안무는 놀라웠지만, 춘절 갈라 로봇 공연은 공장에서 자동차를 조립하는 산업용 로봇에 훨씬 가까운 수준의 자율성과 지능을 보여주었으며, 곧 우리 거실에 등장할 무언가와는 거리가 멀었다. 그럼에도 불구하고 이러한 데모를 보면 로봇 공학이 실제로 어디에 있는지에 대한 의문이 제기된다. 로봇이 쿵푸 동작을 수행하고, 백플립을 하고, 춤을 출 수 있다면, 왜 아직 공장 현장에 등장하지 않는가? 그리고 왜 저녁 식사 후에 내 집에서 설거지를 할 수 없는가? 간단한 대답은 이것이다: 다양한 인간 환경에서 일반적인 작업을 수행할 수 있는 AI 기반 로봇을 만드는 것은 여전히 매우 어렵다. 춘절 갈라와 같은 인상적인 기술적 위업이 우리가 매우 가까이 왔다는 인상을 줄 수 있지만, 이러한 데모에서 AI의 사용은 저수준 모터 제어(로봇이 넘어지지 않도록 하는 것)에만 국한되어 있으며, 따라서 우리 인간이 살고 일하는 실제 비구조적 공간에서 로봇이 범용적으로 사용되기 위한 해결책의 극히 일부에 불과하다.

2. **데이터는 해결되지 않은 과제이다**
   OpenAI의 ChatGPT 및 Anthropic의 Claude와 같은 대규모 언어 모델(LLM)은 처음에 인터넷 규모의 텍스트 데이터베이스로 훈련되었다. 2022년 말, 세상은 ChatGPT가 AI 컴퓨터가 갑자기 산문이나 운문으로, 그리고 거의 모든 주제에 대해 우리에게 "말"할 수 있음을 보여주는 것을 목격했다. LLM은 일반화 능력이 뛰어나며 이제 멀티모달 입력(텍스트, 이미지, 비디오)을 받아 멀티모달 출력을 생성할 수 있다. 중요한 것은 훈련 데이터의 코퍼스가 방대하고 인간이 생성했다는 점인데, 이는 AI 훈련의 황금 기준을 형성하는 특성이다. 일상 생활의 일부로서 로봇으로 가는 가장 빠른 경로는 다양한 로봇 형태가 점점 더 정교한 응용 프로그램을 수행하고 다양한 AI 도구를 사용하는 것에서 비롯될 수 있다. Agility Robotics AI에 몸체(로봇 형태)를 부여하여 물리적 세계에서 사람들과 상호 작용할 수 있게 하는 것은 여전히 매우 어렵고 광범위하게 해결되지 않은 문제이다. 범용 로봇 공학을 위한 AI 모델은 비구조적이고 동적인 환경에서 작동하면서 동시에 여러 가지, 종종 상충되는 물리적, 기하학적, 시간적 제약을 충족해야 한다. 일반화하기 위해 로봇 모델은 고차원 구성 공간에서 수집된 데이터로 훈련되어야 한다. 여기서 "차원"은 텍스트, 조명 조건, 자유도, 관절 제한, 속도, 힘, 안전 경계 등을 나타낸다. 중요한 것은 이것이 좋은 데이터여야 한다는 점이다. 즉, 물리적 세계에서 무한한 수의 가능한 구성에 해당하는 많은 예제를 포함해야 한다. 이러한 종류의 데이터 소스는 거의 없기 때문에 원격 조작, 비디오 분석, 인간의 모션 캡처, 시뮬레이션 및 실제 세계에서의 자체 탐색과 같은 접근 방식이 데이터 수집의 중요한 방법으로 간주된다. 이는 엄청난 작업이다. 예를 들어, Google X의 Everyday Robots에서 우리는 2022년 동안 시뮬레이터에서 2억 4천만 개의 로봇 인스턴스를 실행하여 훈련 데이터를 수집했으며, 주로 쓰레기 분류 모델을 훈련했다. 모든 기술이 유사한 수준의 능력(아직 인간 수준은 아님)에 도달하려면 비슷한 양의 데이터가 필요할 것이다.

3. **단일 로봇 AI는 존재하지 않을 것이다**
   단일 AI 모델이 범용 로봇이 우리와 함께 살고 일할 수 있게 하는 순간은 아직 멀었다. 범용 로봇은 바퀴나 다리를 가질 수 있다. 하나, 둘, 셋 또는 그 이상의 팔을 가질 수 있다. 일부는 프로펠러를 가지고 날 수 있으며, 다른 일부는 수중에서 작동하도록 설계될 수 있다. 일부는 혼잡한 도로를 주행할 것이다. 물리적 세계는 무한히 다양하고 복잡하다. 그리고 모든 사람들이 있다.
