...
summary:
  ...
  zh: CoorDex 是一个学习流水线，它将全身与手腕稳定的手部运动先验知识蒸馏到潜在动作空间中，并训练一个协调的残差 PPO 策略，使配备 20 自由度 WUJI 灵巧手的 Unitree G1 人形机器人能够在行走的同时执行灵巧操作。其核心贡献在于将高维身体与灵巧手控制转化为协调的潜在残差控制，实现了移动中的连续灵巧操作。
...
verification:
  ...
  confidence: medium
  notes: Abstract backfilled by scripts/backfill_paper_abstracts.py from http://arxiv.org/abs/2606.23680v1. [2026-07-29] zh
...
---
## 概述
传统的人形机器人移动操作常简化为“走走停停”的过程，且依赖低自由度末端执行器。CoorDex 通过从模拟的全身与手部演示开始，训练特权运动跟踪教师模型，并将其蒸馏为基于本体感觉的潜在先验知识。这些冻结的先验知识作为下游残差强化学习的动作空间，一个协调的潜在残差策略通过共享任务上下文与独立的身体-手部残差头来组合这些先验，从而在保持自然全身运动的同时提升手指级接触可靠性。实验表明，CoorDex 使 Unitree G1 人形机器人能够在移动中执行非停止的瓶子抓取与搬运、移动中打开冰箱门以及立方体拾取与翻转等灵巧操作。

## 核心内容
### 方法
- **整体架构**：CoorDex 是一个学习流水线，将高维身体与灵巧手控制转化为协调的潜在残差控制。
- **先验蒸馏**：从模拟的全身与手部演示开始，训练特权运动跟踪教师模型，并将其蒸馏为基于本体感觉的潜在先验知识。
- **协调残差策略**：使用冻结的潜在先验作为下游残差强化学习的动作空间，通过共享任务上下文与独立的身体-手部残差头来组合这些先验。

### 实验设置
- **硬件平台**：Unitree G1 人形机器人配备 20 自由度 WUJI 灵巧手。
- **任务**：非停止瓶子抓取与搬运、移动中打开冰箱门、立方体拾取与翻转。

### 关键结果
- **消融实验**：在行走-抓取-搬运任务中，关节空间 PPO、关节空间手部控制以及单一潜在预测在相同奖励预算下均失败，而潜在先验接口与协调残差结构使高维接触丰富的移动操作变得可训练。
- **性能**：CoorDex 成功实现了移动中的连续灵巧操作，包括非停止瓶子抓取与搬运、移动中打开冰箱门以及立方体拾取与翻转。

### 结论
CoorDex 通过将高维身体与灵巧手控制转化为协调的潜在残差控制，使配备 20 自由度 WUJI 灵巧手的 Unitree G1 人形机器人能够在移动中执行灵巧操作。消融实验验证了潜在先验接口与协调残差结构在高维接触丰富移动操作中的有效性。
