...
summary:
  ...
  zh: AMS 是一个统一了敏捷动态技能与极端平衡维持的单一人形机器人控制框架。它由研究团队提出，核心贡献在于利用异构数据（人类运动捕捉与可控合成平衡运动）训练单一策略，并通过混合奖励机制与自适应学习实现多功能控制。
...
verification:
  ...
  confidence: medium
  notes: Abstract backfilled by scripts/backfill_paper_abstracts.py from http://arxiv.org/abs/2511.17373v3. [2026-07-29] zh
...
---
## 概述
AMS 首次将动态运动跟踪与极端平衡维持整合到单一策略中。其关键思路是利用异构数据源：人类运动捕捉数据集提供丰富的敏捷行为，而物理约束的合成平衡运动则捕捉稳定配置。为解决敏捷性与稳定性的优化目标冲突，AMS 设计了混合奖励方案，对所有数据应用通用跟踪目标，同时仅对合成运动注入平衡特定先验。此外，自适应学习策略通过性能驱动采样和运动特定奖励塑造，实现了跨多样运动分布的高效训练。

## 核心内容
### 方法
AMS 的核心创新在于利用异构数据训练单一策略。数据来源包括：
- **人类运动捕捉数据**：提供丰富的敏捷行为（如跳舞、跑步）
- **合成平衡运动**：通过物理约束生成，捕捉极端平衡配置（如叶问蹲）

### 架构
- **混合奖励方案**：对所有数据应用通用跟踪目标，同时仅对合成运动注入平衡特定先验，以协调敏捷性与稳定性的优化目标
- **自适应学习策略**：采用性能驱动采样和运动特定奖励塑造，实现跨多样运动分布的高效训练

### 实验设置
- **仿真验证**：在模拟环境中进行广泛测试
- **真实机器人验证**：在 Unitree G1 人形机器人上部署

### 关键结果
- 单一策略可执行敏捷技能（如跳舞、跑步）
- 零样本执行极端平衡运动（如叶问蹲）
- 实验证明 AMS 是未来人形机器人应用的多功能控制范式
