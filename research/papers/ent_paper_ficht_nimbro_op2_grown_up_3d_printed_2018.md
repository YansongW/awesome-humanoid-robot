...
summary:
  ...
  zh: NimbRo-OP2 是一款面向研究的开源成人尺寸人形机器人，由 NimbRo 团队开发，身高约 135 cm、重 18 kg。其核心贡献在于采用 3D 打印 PA12 外骨骼、基于 COTS 执行器的驱动系统（含外部齿轮与并联机构），以及
    ROS 软件架构，并在 RoboCup 2017 成人组足球赛中夺冠。
...
---
## 概述
NimbRo-OP2 旨在解决大型人形机器人平台成本高、操作复杂、闭源等问题，提供一款经济实惠且完全开源的硬件与软件方案。机器人身高接近成人，能直接与人类环境交互，且重量轻、无需龙门架即可安全操作。其外骨骼通过 3D 打印制成，兼顾轻量化与美观性。该平台在 RoboCup 2017 日本名古屋的比赛中验证了性能，赢得了 Humanoid League AdultSize 足球赛与技术挑战赛。

## 核心内容
### 设计动机
- 小型人形机器人需在缩微环境中交互，且其尺寸会削弱影响大型机器人的关键因素（如惯性、地面接触力）。
- 现有大型平台多存在成本高、维护难、闭源等问题，限制了研究可重复性与社区协作。

### 机械与电子设计
- **尺寸与重量**：身高 135 cm，体重 18 kg，适合人类环境交互。
- **外骨骼**：采用 3D 打印 PA12 材料，实现轻量化与视觉吸引力。
- **驱动系统**：基于 COTS（商用现成）执行器，结合外部齿轮与并联机构（parallel kinematics），提升关节扭矩与精度。
- **电子系统**：集成 ROS 软件栈，支持模块化控制与快速原型开发。

### 实验验证
- **竞赛表现**：在 RoboCup 2017 日本名古屋的 Humanoid League AdultSize 组别中，赢得足球赛与技术挑战赛双料冠军。
- **关键参数**：机器人全开源（硬件与软件），无需龙门架即可安全操作，降低了研究门槛。
...
## 개요
인간형 로봇의 이동, 전신 동작, 개조되지 않은 인간 환경과의 상호작용, 직관적인 인간-로봇 상호작용에서의 다재다능함은 연구 관심을 증가시켰습니다. 여러 소형 플랫폼이 연구용으로 제공되지만, 이들은 상호작용을 위해 소형화된 환경이 필요하며, 종종 로봇의 작은 규모로 인해 대형 로봇에 영향을 미칠 요소들의 영향이 줄어듭니다. 불행히도, 대형 크기 범위의 많은 연구 플랫폼은 가격이 비싸고, 운영, 유지보수 및 수정이 어려우며, 대부분 폐쇄 소스입니다. 본 연구에서는 하드웨어와 소프트웨어 모두에서 저렴하고 완전히 개방된 플랫폼인 NimbRo-OP2X를 소개합니다. 키가 약 135cm이고 무게가 18kg에 불과한 이 로봇은 인간을 위한 환경에서 상호작용할 수 있을 뿐만 아니라, 운영이 쉽고 안전하며, 이를 위해 갠트리가 필요하지 않습니다. 로봇의 외골격은 3D 프린팅되어 가볍고 시각적으로 매력적인 디자인을 제공합니다. 우리는 로봇의 모든 기계적 및 전기적 측면과 함께 잘 확립된 오픈 소스 ROS 소프트웨어의 일부 기능을 제시합니다. NimbRo-OP2X는 일본 나고야에서 열린 RoboCup 2017에서 휴머노이드 리그 어덜트사이즈 축구 대회와 기술 챌린지에서 우승했습니다.
