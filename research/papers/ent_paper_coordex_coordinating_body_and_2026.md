...
summary:
  ...
  ko: CoorDex는 본체 상태와 관절 시퀀스, 접촉력/촉각 신호를 추적 가능한 신체 목표로 변환하고, PPO/RL 정책 훈련, 확산 정책/흐름 매칭, MM-DiT/Transformer
    동작 헤드 훈련 또는 결합 전신 정책을 통해 최종적으로 전신 궤적/동작 시퀀스, 저수준 제어기 목표, 지형/장면 표현을 출력한다. 핵심은 동작 생성을 조건부 생성 문제로 간주하고,
    확산 또는 흐름 매칭을 사용하여 다중 모달 동작 분포에서 실행 가능한 궤적을 샘플링하는 것이다.
domains:
...
verification:
  ...
  notes: 'Structured body generated by scripts/backfill_critical_paper_bodies.py via DeepSeek API from entry
    metadata. | WP1 dedup merge 2026-08-06: merged ent_paper_coordex_coordinating_body_and_2026 into this
