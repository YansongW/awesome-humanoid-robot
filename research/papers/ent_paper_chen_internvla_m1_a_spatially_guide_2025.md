...
verification:
  ...
  confidence: medium
  notes: 'Abstract backfilled by scripts/backfill_paper_abstracts.py from http://arxiv.org/abs/2510.13778v1. [2026-07-29]
...
## 개요
InternVLA-M1은 공간 유도 훈련을 통해 지시 추종 로봇을 확장 가능한 범용 지능으로 이끌기 위해 설계된 통합 로봇 조작 프레임워크입니다. 이 프레임워크는 2단계 파이프라인을 채택합니다: 먼저 230만 개 이상의 공간 추론 데이터로 사전 훈련하여 지시를 시각적 위치와 정렬해 "어디에서 행동할지"를 결정하는 방법을 학습하고, 이후 플러그 앤 플레이 공간 프롬프트를 통한 행동 후훈련으로 몸체 인식 동작을 생성해 "어떻게 행동할지"를 결정합니다. 실험 결과, 이러한 공간 유도 훈련 전략은 SimplerEnv Google Robot, WidowX, LIBERO Franka 등의 벤치마크에서 각각 14.6%, 17%, 4.3%의 안정적인 성능 향상을 가져왔습니다. 또한 팀은 시뮬레이션 엔진을 구축해 24.4만 개의 일반화 가능한 집기-놓기 작업 데이터를 수집했으며, 200개 작업과 3000개 이상의 객체에서 평균 6.2%의 향상을 달성하고 실제 세계의 복잡한 집기-놓기 작업에서 7.3% 향상, 합성 협동 훈련을 통해 보지 못한 객체와 새로운 구성에서 20.6%의 향상을 달성했습니다.

## 핵심 내용
### 방법 아키텍처
InternVLA-M1의 핵심은 **공간 유도 시각-언어-행동 훈련**으로, 공간 위치 파악을 지시와 로봇 동작을 연결하는 핵심 다리로 사용합니다. 2단계 프로세스는 다음과 같습니다:
- **공간 위치 사전 훈련**: 230만 개 이상의 공간 추론 데이터로 훈련하여 지시를 시각적, 비몸체 인식 위치와 정렬하는 방법을 학습하고 "어디에서 행동할지"를 결정합니다. 이 단계는 박스, 점, 궤적의 세 가지 예측 형태를 지원합니다.
- **공간 유도 행동 후훈련**: 플러그 앤 플레이 공간 프롬프트를 통해 몸체 인식 동작을 생성해 "어떻게 행동할지"를 결정합니다. 이 설계는 공간 위치 파악 능력과 동작 생성을 분리하여 모듈식 확장을 용이하게 합니다.

### 실험 설정 및 주요 결과
- **절제 실험**: SimplerEnv Google Robot에서 공간 유도 변형은 비유도 변형보다 14.6% 향상; WidowX에서 17% 향상; LIBERO Franka에서 4.3% 향상.
- **공간 추론 능력**: 박스, 점, 궤적 예측 작업에서 모두 더 강력한 공간 추론 능력을 보여줍니다.
- **시뮬레이션 데이터 확장**: 시뮬레이션 엔진을 구축해 24.4만 개의 일반화 가능한 집기-놓기 작업 데이터를 수집했으며, 200개 작업과 3000개 이상의 객체에서 평균 6.2%의 향상을 달성했습니다.
- **실제 세계 실험**: 복잡한 집기-놓기 작업에서 7.3% 향상; 합성 협동 훈련을 통해 보지 못한 객체와 새로운 구성에서 20.6%의 향상을 달성했습니다.
- **장기 추론**: 복잡한 추론이 필요한 장시간 작업에서 기존 방법을 10% 이상 초과합니다.

### 결론
InternVLA-M1은 공간 유도 훈련이 확장 가능하고 견고한 범용 로봇 정책의 통일된 원리임을 검증했습니다. 코드와 모델은 https://github.com/InternRobotics/InternVLA-M1에서 오픈소스로 공개되었습니다.
