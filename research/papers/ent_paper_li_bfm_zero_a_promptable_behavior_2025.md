...
summary:
  ...
  zh: BFM-Zero 是一个基于无监督强化学习和 Forward-Backward 表征的可提示行为基础模型，用于全身人形机器人控制。由研究团队提出，核心贡献在于通过共享潜在空间实现零样本运动跟踪、目标到达、奖励优化及少样本潜在空间适应，并在真实
    Unitree G1 人形机器人上验证。
...
verification:
  ...
  confidence: medium
  notes: Abstract backfilled by scripts/backfill_paper_abstracts.py from http://arxiv.org/abs/2511.04131v1. [2026-07-29] zh
...
---
## 概述
BFM-Zero 框架通过无监督强化学习和 Forward-Backward 表征学习一个共享的潜在空间，将运动、目标和奖励嵌入同一空间，使单一策略无需重新训练即可提示执行多种下游任务。该模型在真实 Unitree G1 人形机器人上展示了全身技能的多样性和鲁棒性，包括零样本运动跟踪、目标到达、奖励优化以及基于优化的少样本适应。与先前基于策略的强化学习框架不同，BFM-Zero 利用无监督 RL 和 FB 模型提供目标导向、可解释且平滑的全身运动潜在表征，并通过奖励塑形、域随机化和历史依赖非对称学习弥合 sim-to-real 差距。

## 核心内容
### 方法
BFM-Zero 基于无监督强化学习和 Forward-Backward (FB) 模型构建。FB 模型提供目标导向、可解释且平滑的潜在表征，将运动、目标和奖励嵌入共同空间。策略通过离策略无监督 RL 训练，无需任务特定奖励函数。

### 架构
- **潜在空间**：共享潜在空间编码全身运动、目标和奖励，支持多种提示方式。
- **策略**：单一可提示基础策略，通过潜在空间提示执行不同任务。
- **关键扩展**：包括奖励塑形、域随机化和历史依赖非对称学习，以弥合 sim-to-real 差距。

### 实验设置
- **硬件**：真实 Unitree G1 人形机器人。
- **任务**：零样本运动跟踪、目标到达、奖励优化和少样本潜在空间适应。
- **训练**：基于离策略无监督 RL，在仿真中定量消融关键设计选择。

### 关键数字
- 在真实 Unitree G1 上实现零样本运动跟踪、目标到达和奖励优化。
- 少样本适应基于优化方法，在潜在空间中进行。
- 仿真消融实验验证了奖励塑形、域随机化和历史依赖非对称学习的有效性。

### 结论
BFM-Zero 是首个可提示行为基础模型，用于全身人形机器人控制，展示了从仿真到真实世界的可扩展性。它通过单一策略统一多种控制任务，无需重新训练，为未来可提示行为基础模型奠定基础。
