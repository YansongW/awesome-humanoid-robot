...
summary:
  ...
  ko: 'BifrostUMI는 주로 데이터 폐루프(data closed-loop) 문제를 해결한다: 카메라 이미지/다시점(multi-view) 관측, 본체 상태와 관절 시퀀스, 원격
    조작/외골격 데이터를 통해 인간의 조작과 로봇 상태를 수집하고, 이후 ACT/행동 클로닝(behavior cloning) 모방 학습, 확산 정책(diffusion policy)/흐름
    매칭(flow matching), DINO/시각 특징 추출을 통해 훈련 가능하고 재사용 가능한 전신 궤적/동작 시퀀스, 저수준 제어기 목표, 지형/장면 표현으로 변환한다. 핵심 포인트는
