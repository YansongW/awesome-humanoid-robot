...
summary:
  ...
  ko: Agility Meets Stability는 본체 상태와 관절 시퀀스, 인간 비디오/모션 캡처 궤적, 시뮬레이션 상호작용 데이터를 추적 가능한 신체 목표로 변환하고, 이종 모션
    캡처 및 합성 균형 데이터, 혼합 보상과 동적 샘플링, 계층적 스킬/전문가 정책 훈련 또는 조합 전신 정책을 통해 최종적으로 전신 궤적/동작 시퀀스, 저수준 제어기 목표를 출력한다.
    핵심은 민첩한 모션 캡처와 합성 균형 샘플을 동일한 정책에 통합하되, 혼합 보상을 사용하여 동작 추적과 극단적 균형을 각각 제약하는 것이다.
domains:
...
verification:
  ...
  notes: 'Structured body generated by scripts/backfill_critical_paper_bodies.py via DeepSeek API from entry
    metadata. | WP1 dedup merge 2026-08-06: merged ent_paper_agility_meets_stability_versat_2026 into this
