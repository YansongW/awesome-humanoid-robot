...
summary:
  ...
  ko: H2-COMPACT는 먼저 카메라 이미지/다중 시점 관측, 본체 상태와 관절 시퀀스, 시뮬레이션 상호작용 데이터로부터 장면, 대상 또는 운동 표현을 복원한 다음, 확산 정책/플로우
    매칭, 계층적 스킬/전문가 정책을 사용하여 관절 위치/토크 명령, 전신 궤적/동작 시퀀스, 지형/장면 표현을 생성합니다. 핵심은 동작 생성을 조건부 생성 문제로 간주하고, 확산 또는
    플로우 매칭을 통해 다중 모달 동작 분포에서 실행 가능한 궤적을 샘플링하는 것입니다.
domains:
...
verification:
  ...
  notes: 'Structured body generated by scripts/backfill_critical_paper_bodies.py via DeepSeek API from entry
    metadata. | WP1 dedup merge 2026-08-06: merged ent_paper_h2_compact_humanhumanoid_co_ma_2025 into this
