...
summary:
  ...
  ko: '이 연구는 주로 데이터 폐루프(data closed-loop) 문제를 해결한다: 온톨로지 상태와 관절 시퀀스, 원격 조작/외골격 데이터, 시뮬레이션 상호작용 데이터를 통해 인간의
    조작과 로봇 상태를 수집하고, 이후 교사-학생 지식 증류(teacher-student knowledge distillation), 확산 정책(diffusion policy)/흐름
    매칭(flow matching), 전신 제어기(WBC)/MPC를 통해 훈련 가능하고 재사용 가능한 전신 궤적/동작 시퀀스 및 저수준 제어기 목표로 변환한다. 핵심은 특권 정보(privileged
    information)를 사용하여 교사 정책을 훈련하고, 그 능력을 배포 시 관측만 사용할 수 있는 학생 정책으로 증류하는 것이다.'
domains:
