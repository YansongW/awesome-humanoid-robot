...
summary:
  ...
  ko: RPL은 먼저 카메라 이미지/다중 시점 관측, 본체 상태와 관절 시퀀스, 깊이/포인트 클라우드/높이 맵에서 장면, 목표 또는 운동 표현을 복원한 다음, 하향식 깊이 카메라와 U-Net
    높이 맵 재구성, 교사-학생 지식 전이, 세계 모델/비디오 예측을 통해 전신 궤적/동작 시퀀스, 저수준 제어기 목표, 지형/장면 표현을 생성한다. 핵심은 지형 재구성, 보행 위상,
    전신 자세를 동일한 제어 루프에 통합하는 것이지, 인식과 운동 제어를 느슨한 직렬 연결로 분리하는 것이 아니다.
domains:
