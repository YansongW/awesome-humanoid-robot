...
summary:
  en: "arXiv:2606.30749v1 Announce Type: new \nAbstract: Large-scale dexterous grasp datasets encode rich priors over hand-object\
    ...
    \ contact, and operate its functional moving parts. We adapt a hierarchical imitation learning framework that combines\
    \ high-level hand sub-goal prediction with a low-level goal-conditioned controller. We construct a 355k-trajectory grasp-pretraining\
    \ dataset from large-scale dexterous grasp annotations and use it to pretrain the low-level controller. The controller\
    \ is then fine-tuned on downstream task demonstrations. To evaluate this setting, we introduce DexCraft, a simulation\
    \ benchmark with six articulated tool-use tasks requiring coordinated finger motion. Across simulation and real-world\
    \ experiments, our approach outperforms end-to-end diffusion policy baselines and hierarchical policies trained from scratch.\
    \ In the real world, it improves full-task success by 33.3 percentage points over DP3. These results show that grasp datasets\
    \ can serve not only as resources for grasp synthesis, but also as scalable pretraining data for contact-rich dexterous\
    \ manipulation. Videos are shown on https://yingyuan0414.github.io/grasp2dexterity/ ."
