...
summary:
  ...
  ko: 'OmniH2O는 주로 데이터 폐쇄 루프(data closed-loop)를 해결한다: 언어 명령, 카메라 이미지/다중 시점 관측, 인간 비디오/모션 캡처 궤적을 통해 인간 조작과
    로봇 상태를 수집하고, 이후 교사-학생 지식 전이(teacher-student knowledge transfer), PPO/RL 정책 훈련, ACT/행동 클로닝 모방 학습을 통해
    훈련 가능하고 재사용 가능한 전신 궤적/동작 시퀀스 및 저수준 제어기 목표로 변환한다. 핵심은 특권 정보(privileged information)를 사용하여 교사 정책을 훈련하고,
    배포 관측(deployment observation)만 사용할 수 있는 학생 정책으로 능력을 증류(distillation)하는 것이다.'
domains:
