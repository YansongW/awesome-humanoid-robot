...
---
## 概述
该研究针对人形机器人在动态交互任务中的挑战，设计了一套强化学习训练管线，使机器人能协调步法与击球动作。训练采用三阶段课程：先学习步法移动，再生成精准挥拍，最后通过任务导向微调统一全身控制。部署时使用扩展卡尔曼滤波（EKF）估计和预测羽毛球轨迹，并开发了无需预测的简化版本。仿真实验中两个机器人能连续对打21拍，真实世界测试中（包括机器发球和人机对打）出球速度最高19.1 m/s，平均回球落点距离4米。无预测变体性能与基于EKF的目标已知策略相当。

## 核心内容
### 方法架构
- **控制器设计**：统一全身强化学习控制器，协调21自由度人形机器人的步法与击球动作，无需运动先验或专家演示。
- **三阶段训练课程**：
  1. **步法获取**：学习移动至击球位置
  2. **精准挥拍生成**：训练手臂与躯干协调击球
  3. **任务导向精调**：联合优化腿部和手臂动作以最大化击球目标

### 部署方案
- **基于EKF的轨迹预测**：使用扩展卡尔曼滤波估计和预测羽毛球轨迹，用于目标击球
- **无预测变体**：移除EKF和显式预测，性能与基于EKF的目标已知策略相当

### 实验设置
- **仿真实验**：两个机器人连续对打，实现21拍连续击球
- **真实世界测试**：
  - 机器发球测试
  - 人机对打测试
- **关键性能指标**：
...
### 结论
该方法实现了人形机器人打羽毛球时的动态且精准的目标击球，为更动态的关键全身交互任务提供了可行路径。
