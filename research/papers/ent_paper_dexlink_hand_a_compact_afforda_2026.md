---
$id: ent_paper_dexlink_hand_a_compact_afforda_2026
$schema: ../../data/schema/v1/entry_schema.json
$version: 1
type: paper
names:
  en: 'DexLink Hand: A Compact, Affordable, 16-DOF Linkage-Driven Hand with Human-Like
    Dexterity'
  zh: DexLink Hand：紧凑、低成本、16 自由度连杆驱动拟人灵巧手
  ko: ''
summary:
  en: 'DexLink Hand: A Compact, Affordable, 16-DOF Linkage-Driven Hand with Human-Like
    Dexterity is a paper on Hardware Design for humanoid robotics. DexLink Hand：紧凑、低成本、16
    自由度连杆驱动拟人灵巧手.'
  zh: 'DexLink Hand: A Compact, Affordable, 16-DOF Linkage-Driven Hand with Human-Like
    Dexterity is a paper on Hardware Design for humanoid robotics. DexLink Hand：紧凑、低成本、16
    自由度连杆驱动拟人灵巧手.'
  ko: ''
domains:
- 06_design_engineering
- 02_components
layers:
- intelligence
functional_roles:
- knowledge
- intelligence
- system
tags:
- dexlink_hand
- hardware_design
- humanoid
theoretical_depth:
- system
verification:
  status: unverified
  reviewed_by: ai
  reviewed_at: '2026-07-01'
  confidence: low
  notes: 'Imported from Humanoid_Robot_Learning_Paper_Notebooks progress.json. Folder:
    papers/12_Hardware_Design/DexLink_Hand__Compact_Affordable_16-DOF_Linkage-Driven_Hand.
    Route: Hardware Design.'
sources:
- id: src_001
  type: website
  title: 'DexLink Hand: A Compact, Affordable, 16-DOF Linkage-Driven Hand with Human-Like
    Dexterity'
  url: ''
  date: '2026'
  accessed_at: '2026-07-01'
---

DexLink Hand: A Compact, Affordable, 16-DOF Linkage-Driven Hand with Human-Like Dexterity is a paper on Hardware Design for humanoid robotics. DexLink Hand：紧凑、低成本、16 自由度连杆驱动拟人灵巧手.
