...
summary:
  ...
  zh: UniTracker 是一个三阶段学习框架，用于在 Unitree G1 人形机器人上跟踪数千种全身运动。它由特权全观测教师策略、基于 CVAE 的通用学生策略以及轻量级残差解码器适应模块组成，核心贡献在于实现了高泛化性和鲁棒性的运动跟踪。
...
verification:
  ...
  confidence: medium
  notes: Abstract backfilled by scripts/backfill_paper_abstracts.py from http://arxiv.org/abs/2507.07356v3. [2026-07-29] zh
...
---
## 概述
UniTracker 通过三阶段训练流程解决了人形机器人全身运动控制的泛化与鲁棒性问题。第一阶段训练一个具有特权观测的教师策略，生成高质量动作；第二阶段引入条件变分自编码器（CVAE）构建通用学生策略，通过将部分观测先验与全观测编码器对齐来学习全局运动潜在表示，克服了标准 MLP 策略在部分观测下的漂移问题；第三阶段设计轻量级残差解码器适应模块，可对难以直接跟踪的复杂运动序列进行快速微调，支持单序列和批量模式。在 Unitree G1 人形机器人的仿真与真实环境评估中，UniTracker 在运动多样性、跟踪精度和部署鲁棒性上表现优异。

## 核心内容
### 方法架构
UniTracker 采用三阶段训练框架：
- **第一阶段**：训练一个特权全观测教师策略，利用完整状态信息生成高质量动作序列。
- **第二阶段**：引入条件变分自编码器（CVAE）构建通用学生策略。CVAE 学习全局运动潜在表示，通过将部分观测先验与全观测编码器对齐，使策略在部分观测下也能保持全局属性（如朝向）的稳定性，避免标准 MLP 策略的漂移问题。
- **第三阶段**：设计轻量级残差解码器适应模块，对通用策略进行快速微调，专门处理难以直接跟踪的复杂运动序列。该适应支持单序列和批量模式，提升了框架的灵活性与可扩展性。

### 实验设置与关键结果
- **平台**：Unitree G1 人形机器人，在仿真和真实环境中均进行了评估。
- **性能指标**：运动多样性、跟踪精度、部署鲁棒性。
- **关键发现**：UniTracker 在跟踪数千种全身运动时表现出高精度和强鲁棒性，CVAE 结构显著提升了未见行为的泛化能力，残差适应模块有效解决了复杂运动序列的跟踪难题。

### 结论
UniTracker 通过三阶段框架实现了人形机器人全身运动跟踪的泛化与鲁棒性，其 CVAE 学生策略和轻量级适应模块为实际部署提供了可扩展的解决方案。
