---
$id: ent_paper_add_adversarial_disentanglemen_2026
$schema: ../../data/schema/v1/entry_schema.json
$version: 1
type: paper
names:
  en: 'ADD: Adversarial Disentanglement and Distillation'
  zh: 对抗解耦与蒸馏
  ko: 'ADD: Adversarial Disentanglement and Distillation'
summary:
  en: ''
  zh: 'ADD: Adversarial Disentanglement and Distillation is a paper on 风格学习 for humanoid
    robotics. 对抗解耦与蒸馏.'
  ko: 'ADD: Adversarial Disentanglement and Distillation is a paper on 风格学习 for humanoid
    robotics. 对抗解耦与蒸馏.'
domains:
- 07_ai_models_algorithms
layers:
- intelligence
functional_roles:
- knowledge
- intelligence
tags:
- add
- foundation
- humanoid
- reinforcement_learning
theoretical_depth:
- system
verification:
  status: unverified
  reviewed_by: ai
  reviewed_at: '2026-07-01'
  confidence: low
  notes: 'Imported from Humanoid_Robot_Learning_Paper_Notebooks progress.json. Folder:
    papers/01_Foundational_RL/ADD_Adversarial_Differential_Discriminators. Route:
    风格学习.'
sources:
- id: src_001
  type: website
  title: 'ADD: Adversarial Disentanglement and Distillation'
  url: ''
  date: '2026'
  accessed_at: '2026-07-01'
---

## 概述
ADD: Adversarial Disentanglement and Distillation is a paper on 风格学习 for humanoid robotics. 对抗解耦与蒸馏.

## 개요
ADD: Adversarial Disentanglement and Distillation is a paper on 风格学习 for humanoid robotics. 对抗解耦与蒸馏.
