...
summary:
  ...
  ko: HDMI는 먼저 카메라 이미지/다중 시점 관측, 본체 상태와 관절 시퀀스, 인간 비디오/모션 캡처 궤적으로부터 장면, 목표 또는 운동 표현을 복원한 다음, IK/동작 리타게팅,
    계층적 스킬/전문가 정책을 통해 전신 궤적/동작 시퀀스를 생성한다. 핵심은 작업을 라우팅 가능한 스킬 또는 전문가 정책으로 분해하고, 상위 모듈이 실행 중에 이를 선택하고 결합하는
    것이다.
domains:
