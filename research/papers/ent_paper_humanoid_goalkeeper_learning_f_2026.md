...
verification:
  ...
  confidence: low
  notes: 'Imported from WeChat curated list (113). Institution: The University of
    Hong Kong、Shanghai AI Laboratory. Full title: Humanoid Goalkeeper: Learning from
    Position Conditioned Task-Motion Constraints.'
sources:
- id: src_001
  type: website
...
## 概述
Humanoid 主要解决数据闭环：用人类视频/动捕轨迹、遥操作/外骨骼数据、接触力/触觉信号采集人类操作和机器人状态，再通过PPO/RL 策略训练、AMP/运动先验、扩散策略/流匹配转成可训练、可复用的可执行动作命令。关键点是把动作生成看成条件生成问题，用扩散或流匹配在多模态动作分布里采样可执行轨迹。
