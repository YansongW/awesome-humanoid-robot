...
summary:
  ...
  ko: '이 작업은 주로 데이터 폐쇄 루프(data closed-loop)를 해결한다: 카메라 이미지/다중 시점 관측, 본체 상태와 관절 시퀀스, 원격 조작/외골격 데이터를 통해 인간의
    조작과 로봇 상태를 수집하고, 이후 PPO/RL 정책 훈련, 확산 정책(diffusion policy)/흐름 매칭(flow matching), MM-DiT/Transformer 액션
    헤드를 통해 훈련 가능하고 재사용 가능한 엔드 이펙터/손목-손 목표, 액션 청크(chunk)/토큰으로 변환한다. 핵심 포인트는 액션 생성을 조건부 생성 문제로 간주하고, 확산 또는
    흐름 매칭을 사용하여 다중 모드 액션 분포에서 실행 가능한 궤적을 샘플링하는 것이다.'
domains:
