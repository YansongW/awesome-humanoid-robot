...
summary:
  ...
  ko: VOFA는 카메라 이미지/다중 시점 관측, 본체 상태와 관절 시퀀스, 시뮬레이션 상호작용 데이터를 추적 가능한 신체 목표로 변환하고, 확산 정책/플로우 매칭, 전신 제어기/WBC/MPC,
    계층적 스킬/전문가 정책 훈련 또는 전신 정책 조합을 통해 최종적으로 전신 궤적/동작 시퀀스, 저수준 제어기 목표, 내비게이션/도달 목표를 출력한다. 핵심은 동작 생성을 조건부 생성
    문제로 간주하고, 확산 또는 플로우 매칭을 사용하여 다중 모달 동작 분포에서 실행 가능한 궤적을 샘플링하는 것이다.
domains:
