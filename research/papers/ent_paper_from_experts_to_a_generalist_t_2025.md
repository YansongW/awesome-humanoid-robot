...
summary:
  ...
  ko: 이 작업은 카메라 이미지/다중 시점 관측, 본체 상태와 관절 시퀀스, 시뮬레이션 상호작용 데이터를 추적 가능한 신체 목표로 변환하고, 세계 모델/비디오 예측, 전신 제어기/WBC/MPC
    훈련 또는 결합 전신 정책을 통해 최종적으로 전신 궤적/동작 시퀀스와 저수준 제어기 목표를 출력합니다. 핵심은 세계 모델/비디오 예측과 전신 제어기/WBC/MPC를 동일한 훈련/배포
    파이프라인에 통합하여 고수준 목표와 저수준 동작 사이의 단절을 줄이는 것입니다.
domains:
