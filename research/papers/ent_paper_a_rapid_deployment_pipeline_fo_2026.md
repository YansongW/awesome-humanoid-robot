...
summary:
  ...
  ko: 이 작업은 먼저 카메라 이미지/다중 시점 관측, 본체 상태와 관절 시퀀스, 깊이/포인트 클라우드/높이 맵에서 장면, 목표 또는 운동 표현을 복원한 다음, 확산 정책/플로우 매칭,
    IK/동작 리타게팅을 통해 관절 위치/토크 명령, 지형/장면 표현을 생성한다. 핵심은 동작 생성을 조건부 생성 문제로 간주하고, 확산 또는 플로우 매칭을 사용하여 다중 모달 동작
