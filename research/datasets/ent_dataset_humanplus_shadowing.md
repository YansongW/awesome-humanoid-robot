...
summary:
  en: A dataset collected by shadowing human motion with a humanoid robot, used to train whole-body imitation and teleoperation
    policies.
  zh: '核心内容 ### HumanPlus Shadowing数据集的定义与定位 HumanPlus Shadowing数据集属于 **dataset** 类型。 所属领域包括：09_data_datasets。 价值链层级：intelligence。
    通过人形机器人跟随人体运动采集的数据集，用于训练全身模仿与遥操作策略。 英文名称为 *HumanPlus Shadowing Dataset*。 韩文名称为 *HumanPlus Shadowing 데이터셋*。'
