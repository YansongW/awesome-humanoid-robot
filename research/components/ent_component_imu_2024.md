...
## Overview
Inertial Measurement Units are critical components in the field of humanoid robotics. The following content is compiled from the project Wiki for in-depth reference.
...
The ADIS16475 is a precision, miniature MEMS Inertial Measurement Unit (IMU) from Analog Devices, integrating a triaxial gyroscope and a triaxial accelerometer, along with built-in temperature sensors, signal conditioning, calibration, and digital filtering. Each sensor is factory-calibrated for sensitivity, bias, axial alignment, linear acceleration (gyroscope bias), and impact center (accelerometer position), providing high-precision six-degree-of-freedom inertial measurements over a wide temperature range. It is suitable for applications such as robotics, drones, autonomous driving, industrial stabilization platforms, and navigation systems.
...
1. **MEMS Inertial Sensing Elements**: The triaxial gyroscope and triaxial accelerometer share the same package, sensing angular velocity and specific force through precision micro-mechanical structures.
...
3. **Internal Processor and Registers**: Outputs angular velocity, acceleration, temperature, delta angle, and delta velocity data via the SPI interface.
...
- **Angular Random Walk (ARW)**: $0.15°/\sqrt{\text{hr}}$, characterizing gyroscope angle integration noise:
  ...
  \sigma_\theta(t) = \text{ARW} \cdot \sqrt{t}
  $$
- **In-Run Bias Stability**: Gyroscope typical value $2°/\text{hr}$, accelerometer $3.6~\mu g$.
- **Cross-Axis Alignment Error**: $\pm 0.1°$, significantly reducing installation alignment effort in multi-sensor fusion.
...
- **Humanoid Robot Attitude Estimation**: Provides high-frequency inertial reference for the torso, feet, or head, fusing with vision and joint encoders for stable attitude estimation.
- **Drone Flight Control**: Used for attitude stabilization, heading hold, and navigation calculations.
- **Autonomous Driving and Robot Navigation**: Fuses with GNSS/LiDAR/vision for high-dynamic pose estimation.
...
Key parameters are sourced from the official Analog Devices ADIS16475 datasheet (Rev. C) and the DigiKey authorized distributor page. Critical specifications such as gyroscope range, bias stability, ARW, cross-axis alignment error, supply voltage, and temperature range are clearly stated in the official datasheet. Price is not publicly available.
...
4. **자체 테스트 및 동기화**: 요청 시 자체 테스트, 플래시 자체 테스트, 데이터 준비 표시 및 외부 동기화 모드(direct/pulse/scaled/output)를 지원합니다.
...
- **각 랜덤 워크(ARW)**: $0.15°/\sqrt{\text{hr}}$로, 자이로 각도 적분 잡음을 나타냅니다:
  ...
  \sigma_\theta(t) = \text{ARW} \cdot \sqrt{t}
  $$
- **운전 중 바이어스 안정성**: 자이로스코프 일반값 $2°/\text{hr}$, 가속도계 $3.6~\mu g$.
...
| 운전 중 자이로 바이어스 안정성 | 2 °/hr(일반) | ADI 데이터시트 |
| 각 랜덤 워크 | 0.15 °/√hr | ADI 데이터시트 |
| 가속도계 운전 바이어스 안정성 | 3.6 μg | ADI 데이터시트 |
| 축 간 정렬 오차 | ±0.1° | ADI 데이터시트 |
| 출력 데이터 | 각속도, 가속도, 온도, 델타 각도, 델타 속도 | ADI 데이터시트 |
| 인터페이스 | SPI | ADI 데이터시트 |
| 공급 전압 | 3.0 V – 3.6 V | ADI 데이터시트 |
| 작동 전류 | 약 44 mA(일반) | 유통업체 |
| 작동 온도 | −40 °C ~ +105 °C | ADI 데이터시트 |
...
주요 매개변수는 Analog Devices 공식 ADIS16475 데이터시트(Rev. C) 및 DigiKey 공인 유통업체 페이지에서 가져왔습니다. 자이로스코프 범위, 바이어스 안정성, ARW, 축 간 정렬 오차, 공급 전압 및 온도 범위 등 주요 지표는 공식 데이터시트에 명확히 명시되어 있습니다. 가격은 비공개입니다.
