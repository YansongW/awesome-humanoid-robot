---
$id: ent_report_unitree_unitree_b_w_showcased_at_icra_2024
$schema: ../../data/schema/v1/entry_schema.json
$version: 1
type: report
names:
  en: Unitree B-W Showcased at ICRA 2023 London
  zh: Unitree B-W Showcased at ICRA 2023 London
  ko: Unitree B-W Showcased at ICRA 2023 London
summary:
  en: Unitree Robotics participated in ICRA for the sixth consecutive year. Unitree Robotics exhibited Go1, B1, A1, and other
    quadruped robots at ICRA. Unitree is a world-renowned robotics company that focuses on self-development. LONDON, ENGLAND
    / ACCESSWIRE / June 2, 2023 /&nbsp; On May 29, ICRA 2023, the International Conference on Robotics and Automation, was
    held in London, UK, which is the largest and most influential international academic conference in the field of robotics
    organized by the In
  zh: Unitree Robotics 在伦敦举办的 ICRA 2023 上展示了其四足机器人产品线，包括 Go1、B1 和 A1。这是该公司连续第六年参加这一机器人领域最具影响力的国际学术会议。Unitree 是一家专注于自主研发的全球知名机器人公司。
  ko: Unitree Robotics participated in ICRA for the sixth consecutive year. Unitree Robotics exhibited Go1, B1, A1, and other
    quadruped robots at ICRA. Unitree is a world-renowned robotics company that focuses on self-development. LONDON, ENGLAND
    / ACCESSWIRE / June 2, 2023 /&nbsp; On May 29, ICRA 2023, the International Conference on Robotics and Automation, was
    held in London, UK, which is the largest and most influential international academic conference in the field of robotics
    organized by the In
domains:
- 11_applications_markets
- 06_design_engineering
layers:
- midstream
- validation_markets
functional_roles:
- knowledge
- market
tags:
- company_news
- robotics
- unitree
theoretical_depth:
- system
verification:
  status: partially_verified
  reviewed_by: ai
  reviewed_at: '2026-07-01'
  confidence: medium
  notes: 'Imported from Unitree official news API. News ID: 25. [2026-07-29] zh content backfilled from English abstract via
    scripts/sinicize_english_cards.py'
sources:
- id: src_001
  type: website
  title: Unitree B-W Showcased at ICRA 2023 London
  url: https://www.unitree.com/mobile/news
  date: '2024-04-19'
  accessed_at: '2026-07-01'
---
## 概述
在 2023 年 5 月 29 日于英国伦敦举行的 ICRA 2023（国际机器人与自动化会议）上，Unitree Robotics 连续第六年参展。该公司展出了 Go1、B1、A1 等多款四足机器人，展示了其在机器人自主研发领域的持续投入与创新成果。ICRA 是由 IEEE 主办的机器人领域规模最大、最具影响力的国际学术会议。

## 核心内容
### 参展概况
- Unitree Robotics 在 ICRA 2023 上连续第六年亮相，展示了其核心四足机器人产品线。
- 展出的机器人型号包括 Go1、B1 和 A1，均为该公司自主研发的成果。

### 会议背景
- ICRA 2023 于 2023 年 5 月 29 日在英国伦敦举办，由 IEEE 组织。
- 该会议是机器人领域规模最大、影响力最广的国际学术盛会。

### 公司定位
- Unitree 是一家全球知名的机器人公司，专注于核心技术的自主研发。

## Overview
Unitree Robotics participated in ICRA for the sixth consecutive year. Unitree Robotics exhibited Go1, B1, A1, and other quadruped robots at ICRA. Unitree is a world-renowned robotics company that focuses on self-development. LONDON, ENGLAND / ACCESSWIRE / June 2, 2023 /&nbsp; On May 29, ICRA 2023, the International Conference on Robotics and Automation, was held in London, UK, which is the largest and most influential international academic conference in the field of robotics organized by the In

## 参考
- https://www.unitree.com/mobile/news

## 개요
2023년 5월 29일 영국 런던에서 열린 ICRA 2023(국제 로봇공학 및 자동화 컨퍼런스)에서 Unitree Robotics가 6년 연속 참가했습니다. 이 회사는 Go1, B1, A1 등 여러 대의 사족 로봇을 전시하며 로봇 자체 개발 분야에서의 지속적인 투자와 혁신 성과를 선보였습니다. ICRA는 IEEE가 주최하는 로봇 분야에서 가장 크고 영향력 있는 국제 학술 컨퍼런스입니다.

## 핵심 내용
### 참가 개요
- Unitree Robotics는 ICRA 2023에서 6년 연속으로 참가하여 핵심 사족 로봇 제품 라인을 선보였습니다.
- 전시된 로봇 모델에는 Go1, B1, A1이 포함되며, 모두 해당 회사의 자체 개발 성과입니다.

### 컨퍼런스 배경
- ICRA 2023은 2023년 5월 29일 영국 런던에서 개최되었으며, IEEE가 주최했습니다.
- 이 컨퍼런스는 로봇 분야에서 가장 크고 영향력이 넓은 국제 학술 행사입니다.

### 회사 포지셔닝
- Unitree는 핵심 기술의 자체 개발에 주력하는 글로벌 로봇 회사입니다.
