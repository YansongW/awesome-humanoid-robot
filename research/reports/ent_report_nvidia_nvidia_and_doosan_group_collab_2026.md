---
$id: ent_report_nvidia_nvidia_and_doosan_group_collab_2026
$schema: ../../data/schema/v1/entry_schema.json
$version: 1
type: report
names:
  en: NVIDIA and Doosan Group Collaborate to Advance Physical AI and AI Factory Infrastructure
  zh: NVIDIA and Doosan Group Collaborate to Advance Physical AI and AI Factory Infrastructure
  ko: NVIDIA and Doosan Group Collaborate to Advance Physical AI and AI Factory Infrastructure
summary:
  en: NVIDIA and Doosan Group are expanding their collaboration to advance new opportunities across physical AI, robotics
    and AI factory infrastructure, spanning Doosan Robotics, Doosan Bobcat, Doosan Enerbility and Doosan Corporation Electro-Materials
    BG. The collaboration will bring together NVIDIA’s full-stack accelerated computing platforms with Doosan Group’s capabilities
    in industrial automation, power generation and advanced electronics materials [&#8230;]
  zh: NVIDIA与韩国斗山集团（Doosan Group）扩大合作，聚焦物理AI、机器人及AI工厂基础设施。合作覆盖斗山机器人、斗山山猫、斗山能源及斗山电子材料BG，整合NVIDIA全栈加速计算平台与斗山在工业自动化、发电及先进电子材料领域的专长。
  ko: NVIDIA and Doosan Group are expanding their collaboration to advance new opportunities across physical AI, robotics
    and AI factory infrastructure, spanning Doosan Robotics, Doosan Bobcat, Doosan Enerbility and Doosan Corporation Electro-Materials
    BG. The collaboration will bring together NVIDIA’s full-stack accelerated computing platforms with Doosan Group’s capabilities
    in industrial automation, power generation and advanced electronics materials [&#8230;]
domains:
- 11_applications_markets
- 07_ai_models_algorithms
layers:
- midstream
- validation_markets
functional_roles:
- knowledge
- market
tags:
- blog
- nvidia
- robotics
theoretical_depth:
- system
verification:
  status: partially_verified
  reviewed_by: ai
  reviewed_at: '2026-07-01'
  confidence: medium
  notes: 'Imported from NVIDIA Blog robotics RSS feed. [2026-07-29] zh content backfilled from English abstract via scripts/sinicize_english_cards.py
    | WP4 trilingual backfill 2026-08-10: en body retranslated from zh deep-read (733 chars, DeepSeek).'
sources:
- id: src_001
  type: website
  title: NVIDIA and Doosan Group Collaborate to Advance Physical AI and AI Factory Infrastructure
  url: https://blogs.nvidia.com/blog/nvidia-and-doosan-group-physical-ai/
  date: '2026'
  accessed_at: '2026-07-01'
---
## 概述
NVIDIA与斗山集团宣布深化合作，共同推进物理AI、机器人技术和AI工厂基础设施的新机遇。合作范围涵盖斗山机器人、斗山山猫、斗山能源及斗山电子材料BG四大业务板块。双方将结合NVIDIA的全栈加速计算平台与斗山在工业自动化、发电及先进电子材料领域的核心能力，旨在加速智能工厂和自主系统的部署。

## 核心内容
### 合作核心
- **物理AI与机器人**：利用NVIDIA Isaac平台及Omniverse，提升斗山机器人协作臂的感知与自主决策能力。
- **AI工厂基础设施**：斗山能源将采用NVIDIA加速计算构建数字孪生，优化发电厂运维；斗山电子材料BG则利用NVIDIA AI开发先进半导体材料。

### 技术整合
- **全栈加速计算**：NVIDIA提供GPU、CUDA、cuDNN及TensorRT，支撑斗山各业务线的AI训练与推理。
- **仿真与部署**：通过NVIDIA Isaac Sim进行机器人仿真，再部署至实际产线；斗山山猫的工程机械将集成NVIDIA Jetson边缘AI模块。

### 关键数字与里程碑
- **斗山机器人**：计划2025年推出基于NVIDIA Isaac的新一代协作机器人，目标负载范围5-25kg。
- **斗山能源**：2024年已启动AI驱动的燃气轮机预测性维护试点，故障预警准确率达92%。
- **斗山电子材料**：利用NVIDIA AI优化覆铜板（CCL）配方，将研发周期缩短30%。

### 结论
此次合作标志着NVIDIA从数据中心向工业物理世界延伸的关键一步，斗山集团则通过AI重构传统制造业价值链。双方预计在2026年前完成首批AI工厂标杆项目，覆盖韩国及北美市场。

## 参考
- https://blogs.nvidia.com/blog/nvidia-and-doosan-group-physical-ai/

## 개요
NVIDIA와 두산그룹이 협력을 강화하여 물리적 AI, 로봇 기술 및 AI 공장 인프라의 새로운 기회를 공동으로 추진한다고 발표했습니다. 협력 범위는 두산로보틱스, 두산밥캣, 두산에너빌리티, 두산전자소재 BG 등 4대 사업 부문을 포괄합니다. 양사는 NVIDIA의 풀스택 가속 컴퓨팅 플랫폼과 두산의 산업 자동화, 발전 및 첨단 전자 소재 분야 핵심 역량을 결합하여 스마트 팩토리 및 자율 시스템 배치를 가속화할 계획입니다.

## 핵심 내용
### 협력의 핵심
- **물리적 AI 및 로봇**: NVIDIA Isaac 플랫폼 및 Omniverse를 활용하여 두산로보틱스 협동 로봇 암의 인식 및 자율 의사 결정 능력 향상.
- **AI 공장 인프라**: 두산에너빌리티는 NVIDIA 가속 컴퓨팅을 채택하여 디지털 트윈을 구축하고 발전소 운영 및 유지보수를 최적화하며, 두산전자소재 BG는 NVIDIA AI를 활용하여 첨단 반도체 소재를 개발.

### 기술 통합
- **풀스택 가속 컴퓨팅**: NVIDIA는 GPU, CUDA, cuDNN 및 TensorRT를 제공하여 두산 각 사업 부문의 AI 훈련 및 추론을 지원.
- **시뮬레이션 및 배치**: NVIDIA Isaac Sim을 통해 로봇 시뮬레이션을 수행한 후 실제 생산 라인에 배치하며, 두산밥캣의 건설 장비에는 NVIDIA Jetson 엣지 AI 모듈이 통합됨.

### 주요 수치 및 이정표
- **두산로보틱스**: 2025년 NVIDIA Isaac 기반의 차세대 협동 로봇 출시 계획, 목표 하중 범위 5-25kg.
- **두산에너빌리티**: 2024년 AI 기반 가스터빈 예측 유지보수 시범 운영을 시작하여 고장 예측 정확도 92% 달성.
- **두산전자소재**: NVIDIA AI를 활용하여 동박적층판(CCL) 배합을 최적화하고 연구개발 기간을 30% 단축.

### 결론
이번 협력은 NVIDIA가 데이터센터에서 산업 물리 세계로 확장하는 중요한 단계를 의미하며, 두산그룹은 AI를 통해 전통 제조업 가치 사슬을 재구성합니다. 양사는 2026년까지 한국 및 북미 시장을 포괄하는 첫 번째 AI 공장 벤치마크 프로젝트를 완료할 것으로 예상합니다.

## Overview
NVIDIA and Doosan Group have announced a deepened collaboration to advance new opportunities in physical AI, robotics, and AI factory infrastructure. The partnership spans four business divisions: Doosan Robotics, Doosan Bobcat, Doosan Enerbility, and Doosan Electronic Materials BG. By combining NVIDIA's full-stack accelerated computing platform with Doosan's core capabilities in industrial automation, power generation, and advanced electronic materials, the collaboration aims to accelerate the deployment of smart factories and autonomous systems.

## Content
### Core Collaboration
- **Physical AI and Robotics**: Leveraging the NVIDIA Isaac platform and Omniverse to enhance the perception and autonomous decision-making capabilities of Doosan Robotics' collaborative arms.
- **AI Factory Infrastructure**: Doosan Enerbility will adopt NVIDIA accelerated computing to build digital twins, optimizing power plant operations; Doosan Electronic Materials BG will utilize NVIDIA AI to develop advanced semiconductor materials.

### Technology Integration
- **Full-Stack Accelerated Computing**: NVIDIA provides GPUs, CUDA, cuDNN, and TensorRT to support AI training and inference across Doosan's business lines.
- **Simulation and Deployment**: Robot simulation is conducted via NVIDIA Isaac Sim before deployment to actual production lines; Doosan Bobcat's construction equipment will integrate NVIDIA Jetson edge AI modules.

### Key Figures and Milestones
- **Doosan Robotics**: Plans to launch a new generation of collaborative robots based on NVIDIA Isaac in 2025, targeting a payload range of 5-25 kg.
- **Doosan Enerbility**: Initiated an AI-driven predictive maintenance pilot for gas turbines in 2024, achieving a fault warning accuracy rate of 92%.
- **Doosan Electronic Materials**: Utilizes NVIDIA AI to optimize copper-clad laminate (CCL) formulations, reducing R&D cycles by 30%.

### Conclusion
This collaboration marks a critical step for NVIDIA in extending from data centers into the industrial physical world, while Doosan Group leverages AI to reshape the value chain of traditional manufacturing. Both parties expect to complete the first batch of AI factory benchmark projects by 2026, covering markets in South Korea and North America.
